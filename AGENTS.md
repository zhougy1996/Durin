...
- Agents must run in a dedicated checkout/worktree and use `Engine/Scripts/Build/agent_build.py` for editor builds and automated validation; `AgentBuild.ps1` is the Windows compatibility wrapper. The worktree owns its build tree, generated metadata, and binary outputs. Do not run an Agent build in a checkout concurrently owned by a human or another workflow.
