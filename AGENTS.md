...
- Agent Build Profiles own isolated DHT metadata under `Engine/Intermediate/Build-<Identifier>/<Platform>/<Profile>/`, such as `Engine/Intermediate/Build-Agent/Win64/DurinEditor/`. DHT serializes commands that intentionally share the same build identifier, platform, and profile; do not bypass the registered Agent workflow or write another workflow's intermediate directory directly.
