...
- Each checkout has one source/build writer at a time; use separate worktrees only for concurrent workflows. On Windows, all Agent `configure`, `build`, `clean`, `purge`, `rebuild`, and `test` actions use root-level `BuildTool.bat`; on other hosts, use `Engine/Scripts/Build/durin_build_tool/__main__.py`.
