...
- Agent Build Profiles own isolated DHT metadata under `Engine/Intermediate/Build/<Platform>/<Config>-<Identifier>/<Profile>/`, such as `Win64/Debug-Agent/DurinEditor/`. DHT serializes commands that intentionally share the same platform, output configuration, and profile; do not bypass the registered Agent workflow or write another profile's intermediate directory directly.
