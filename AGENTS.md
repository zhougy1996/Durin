...
- Treat `BuildTool configure`, `build`, `clean`, `purge`, `rebuild`, and `test` as potentially long-running build tasks: use a long timeout and keep waiting on the same invocation. A yielded or still-running tool call is not an interruption; use `rebuild --target all` only after the command was actually terminated and its interruption marker remains.
