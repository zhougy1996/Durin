...
- Agent Build Profiles with a non-empty build identifier also own isolated DHT metadata under `Engine/Intermediate/Build/<Platform>/<Profile>/<Identifier>/`. DHT serializes commands that intentionally share the same platform, profile, and identifier; do not bypass the registered Agent workflow or write another profile's intermediate directory directly.
