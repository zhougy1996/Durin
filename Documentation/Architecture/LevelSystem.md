...
At startup the editor can restore the most recently activated level or always show the startup level picker. Recent levels are stored per virtual content mount in a user session YAML beside the editor executable. Missing or invalid session data and failed level loads are non-fatal and fall back to the picker.
