...
that module. `MainFrame` derives the default host layout and Window menu from
the registered descriptors; it does not name concrete editor root windows.
...
discarding, releasing, and drawing its resource. `FEditorWorkspaceRootWindow`
provides the shared root-window state transition, while
`FEditorWorkspaceDocumentHost` composes that transition across the per-resource
documents used by Material and Texture editors. The singleton Level workspace
keeps its specialized internal dock-space lifecycle.
...
project transitions, and are cleared at the explicit PIE lifecycle boundaries.
Class-filtered asset selection uses the `DurinEd` asset picker, whose optional
path prefix limits candidate enumeration without retaining a loaded current
selection.
...
modules. Rendering dependencies belong in an editor module only when its
preview implementation actually uses them.
