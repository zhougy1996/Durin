# DurinDevTool Windows Lock Recovery Boundary
...
The checkout ownership mechanism is not Windows-only. DurinDevTool needs one
...
prevents DurinDevTool-owned descriptors and handles from leaking to CMake, Ninja,
...
DurinDevTool lock” error and does not use the ACL-specific recovery explanation.
...
The stale-file recovery is safe only if a live DurinDevTool's Python file handle
...
renamed, while a live DurinDevTool file cannot. The existing automated tests mock
...
- A normal process exit releases the byte lock even though `checkout.lock`
  remains on disk.
- Waiting for the Windows job's active-process count keeps `DurinDevTool run`
  alive until the last relaunched editor process exits.
- `close_fds=True` prevents child-process lifetime from extending DurinDevTool's
...
- The documented manual recovery enables inheritance recursively and then
  ...
  DurinDevTool, DurinEditor, CMake, and Ninja have exited before deletion.
...
3. A Windows integration test proves that a live owner blocks stale-file rename
   ...
   ownership.
4. Existing lock exclusivity, ACL best-effort, child-handle, and DurinDevTool
