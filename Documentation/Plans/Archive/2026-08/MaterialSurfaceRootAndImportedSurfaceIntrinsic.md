...
Implemented and qualified. Material schema 3 owns an optional aggregate Surface
source, normalized IR v2 owns a special literal-or-expression Surface Root, and
StandardSurface persists and normalizes as one aggregate expression. The
derived Material Output remains presentation-only. The exact legacy built-in
fingerprint migrates only `/Engine/Materials/ImportedSurface`; its checked-in
package now contains the compact graph and retains the stable path and parameter
identities.

Qualification evidence:

- default material: 0 authored nodes, 0 ordinary IR nodes, 8 inline root literals;
- legacy canonical versus StandardSurface: 66/66 authored/IR nodes to 1/1,
  canonical IR 3,286 to 255 bytes, generated source 10,825 to 9,973 bytes;
- representative measured compiler phases: normalization 919 to 109 us and
  generation 438 to 50 us; timing is diagnostic, not a GPU-performance claim;
- cooked DMAT bytes remained effectively flat at 137,088 versus 137,277;
- ImportedSurface package size changed from 21,239 to 12,904 bytes;
- passed full `all` build, MaterialTests 106/106, SceneImportTests 4/4,
  MaterialThumbnailTests 7/7, MaterialVulkanTests 1/1,
  SceneImportVulkanTests 1/1, TerrainRenderVulkanTests 1/1, and
  SkeletalMeshRenderResourcesVulkanTests 1/1.
...
- [x] Add `FMaterialIRSurfaceInput` and `FMaterialIRSurfaceRoot` with explicit
  literal, per-property expression, and aggregate expression invariants.
- [x] Replace `FMaterialIR::SurfaceOutputs` index storage with the special root
  while keeping `FMaterialIRNode` a single-result value node.
- [x] Normalize unconnected authored outputs directly into root literals instead
  of appending Constant IR nodes.
- [x] Update canonical encoding, bounds checking, equality, hashing, identity,
  memory accounting, compile-result ownership, and failure diagnostics.
- [x] Update Slang generation to accept zero ordinary nodes, resolve inline root
  literals, and assemble `FMaterialSurface` only at the terminal boundary.
- [x] Update Cooked Program validation/versioning so cooked payloads remain free
  of authored graph state and reject mismatched IR/compiler versions.
- [x] Replace the default-material test expectation of eight constant IR nodes
...
- [x] Add the `Surface` value category and `StandardSurface` opcode without
  allowing Surface values in arithmetic, conversion, texture, or ordinary
  property-link positions.
- [x] Add the aggregate source to authored surface outputs and enforce the
  aggregate/per-property exclusivity rule in bounded validation.
- [x] Implement the one-version schema-2 to schema-3 upgrade before current
  schema validation and keep save output canonical at schema 3.
- [x] Implement the shared Standard Surface descriptor and use it for opcode
  validation, result typing, canonical built-in parameter dependencies, and
  deterministic normalization.
- [x] Lower Standard Surface as one aggregate IR expression consumed by the
  Surface Root; do not materialize its implementation as authored nodes or
  generic normalized nodes.
- [x] Generate the existing PBR computations and bindings from the intrinsic,
  retaining exact finite-value, channel, normal, roughness, emissive, UV, and
  sampler semantics.
- [x] Update dependency inspection so base Details and material instances expose
  precisely the canonical declarations reachable through the intrinsic.
- [x] Add invalid-graph coverage for mixed output modes, Surface type misuse,
...
- [x] Add Standard Surface to graph inspection and the closed creation catalog
  with a distinct aggregate Surface pin type and deterministic title/category.
- [x] Extend graph commands with atomic connect, disconnect, and explicit mode
  switching for the aggregate Material Output input.
- [x] Preserve the derived, non-copyable Material Output terminal while showing
  either the aggregate Surface row or the eight per-property rows according to
  the current mode and connection gesture.
- [x] Extend geometry, link hit testing, semantic zoom, framing, deterministic
  layout, selection adjacency, and diagnostic navigation for Surface links.
- [x] Extend clipboard, duplicate, replace, removal, Undo/Redo, and structured
  inspection tests without serializing the derived terminal itself.
- [x] Ensure disconnecting or deleting Standard Surface returns atomically to
  the retained eight per-property fallbacks rather than creating hidden nodes.
- [x] Add canvas rendering and save/reload coverage showing that the compact
...
- [x] Replace `MakeCanonicalMaterialProgram` as the creation path for
...
- [x] Migrate the checked-in ImportedSurface package explicitly and validate
  that all generated material-instance parent references remain unchanged.
- [x] Restrict legacy canonical-graph recognition to an exact, versioned
  fingerprint in the built-in migration tool; never collapse similar or edited
  user graphs.
- [x] Make `EnsureImportedSurfaceMaterial` validate the expected template
...
- [x] Verify scene import/reimport, opaque/masked/translucent instances, every
  texture semantic and derivation, thumbnails, preview, StaticMesh,
  SkeletalMesh, Terrain, forward, GBuffer, and shadow paths.
- [x] Verify opening ImportedSurface presents exactly one editable Standard
  Surface node plus the derived Material Output terminal and does not synthesize
  the legacy DAG.
- [x] Remove the expanded canonical builder only after all production and test
...
- [x] Run focused material schema/compiler, graph operations, material instance,
  scene import, Cook, thumbnail, and Vulkan rendering tests according to the
  repository testing workflow.
- [x] Run the affected Engine, MaterialEditor, AssetForgeBuiltins, Renderer, and
  RenderCore build targets according to the repository build workflow.
- [x] Record before/after authored-node count, normalized ordinary-node count,
...
- [x] Update the authoritative Material System contract with the Surface Root,
  schema transition, intrinsic lowering, dependency, identity, and Cook rules.
- [x] Update Material Graph Operations with aggregate pin, command, layout,
  clipboard, diagnostic, and presentation behavior.
- [x] Update the Material System roadmap only if milestone status or child-plan
  sequencing changes; do not duplicate the implementation contract there.
- [x] Close this plan only after all acceptance gates pass and lasting behavior
...
- [x] A fresh material has zero authored expression nodes, zero ordinary IR
  nodes, and one literal Surface Root covering all eight outputs.
- [x] A legacy per-property graph upgrades without changing node GUIDs, links,
  fallback literals, presentation, static properties, or rendered behavior.
- [x] Standard Surface persists and normalizes as one aggregate expression; its
  implementation is not reconstructed as ordinary authored or IR nodes.
- [x] Aggregate and per-property output sources cannot coexist in a valid
  program.

### ImportedSurface

- [x] The stable `/Engine/Materials/ImportedSurface` path and canonical
  parameter GUIDs are unchanged.
- [x] Existing generated material instances resolve the same parent and retain
  all value, texture, UV, sampler, alpha, and static-property overrides.
- [x] Opening ImportedSurface shows one Standard Surface node and one derived
  Material Output terminal.
- [x] Only the exact recognized built-in legacy graph is eligible for migration;
  arbitrary user graphs are never rewritten.

### Compiler and runtime

- [x] Canonical encoding is deterministic across authored node order and GUID
  changes where the existing contract requires it.
- [x] Version changes invalidate stale DDC and cooked artifacts, while warm
  compiles reuse the new identity normally.
- [x] Default and Standard Surface generated shaders expose no unused material
  resource bindings after backend reflection.
- [x] StaticMesh, SkeletalMesh, Terrain, preview, thumbnail, forward, GBuffer,
  masked shadow, and translucent behavior remain qualified.
- [x] Failures remain bounded and identify the aggregate root, intrinsic node,
  input, or program without partial publication.

### Editor and automation

- [x] Canvas and structured commands share the same aggregate creation,
  connection, disconnection, replacement, deletion, layout, and inspection
  semantics.
- [x] Copy/paste and Undo/Redo preserve aggregate programs without copying or
  persisting the derived Material Output terminal.
- [x] Frame, semantic zoom, hit testing, diagnostics, and save/reload work with
