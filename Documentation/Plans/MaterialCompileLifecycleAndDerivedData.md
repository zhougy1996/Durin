...
This plan is selected and prepared. Material System roadmap milestone 5 has
delivered its immutable authored-program snapshot, normalized IR, deterministic
program identity, synchronous compiler entry point/result, Renderer binding
seam, fixed-schema transition policy, and measured compile/artifact baseline.
Stage 0 may now freeze the asynchronous lifecycle over that landed boundary.
...
No implementation checklist is complete. The M5 handoff below is evidence for
Stage 0, not an implicit choice of request generations, last-known-good state,
material DDC, Cook payload, or cancellation policy.
...
- Schema/IR/generator/compiler-envelope/pass-contract versions are all 1.
