...
assignment, physical RHI submission scopes, Vulkan cross-queue waits and
release/acquire handoffs, terminal joins, retained payload ownership and
Renderer pool retirement proofs. Native fixtures have exercised shader dispatch,
...
and backend-only physical coordinates. Its 2026-09-17 evidence records an `all`
build and all 22 affected test targets passing, including 93 RHI
command/completion cases and 100 Vulkan integration cases. That migration does
...
They describe the code at each checkpoint, including superseded limitations
and API names, rather than the current implementation or fresh validation.
...
- Preserve the current barrier records as logical data. Extend them or add a
  ...
  the compiled plan.
- Represent public GPU completion with stable owning `FRHIGPUSyncPointRef`
  identities. Device generation, physical queue and timeline coordinates remain
  backend-only. CPU replay completion and logical graph positions are distinct.
- Retirement prerequisites retain exact logical sync-point uses. Validate state
  and authority before any backend-native wait reduction; numeric maxima must
  not hide canceled/failed signals or replace cross-queue completion evidence.
...
- [x] Record selected API boundaries, backend capability requirements and test
  selections; use the completed sync-point migration for current API authority.
- [ ] Measure a representative performance baseline and freeze acceptable
  regression thresholds. The 2026-09-11 operator exception permits implementation
  before measurement; it does not waive final performance acceptance.
...
The table preserves the Stage 0 pre-migration audit and selected ownership
boundaries. Its first column is historical evidence, not a description of the
current source. Implemented contracts are owned by the linked runtime documents;
the stage checklists track remaining qualification. All source paths in the table
are relative to `Engine/Source/Runtime/`.
...
ticket/receipt API in historical checkpoints: callers retain one `FRHIGPUSyncPointRef` throughout
...
These selected API boundaries record the plan's intended shape, updated for
sync-point ownership. Exact implemented signatures and readiness semantics are
owned by [RHI command execution](../Runtime/Rendering/RHICommandExecution.md)
and [Render Graph](../Runtime/Rendering/RenderGraph.md); verify their headers
before an API migration rather than treating this inventory as a second contract.
...
Dependencies: Stage 0 contract decisions. Performance measurement is explicitly
deferred by the recorded operator exception.
...
These checkpoint records preserve original validation provenance. Present-tense
statements, ticket/receipt terminology and outstanding-work notes in this section
are historical. Use Current Status and the stage checklists for remaining work;
use the linked runtime contracts for implemented API authority. Passing results
apply only to their recorded revision and configuration. The source-only review
on 2026-09-21 adds no native execution evidence.

### Status History Through 2026-09-17
