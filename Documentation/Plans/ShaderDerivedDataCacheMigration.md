...
started. RenderCore currently owns one filesystem `FShaderCacheStore` that
persists both machine-local dependency manifests and portable compiled
artifacts beneath `DerivedDataCache/Shaders/SPIR-V`. The compile service checks
its bounded in-memory output cache, reads per-entry-point SPIR-V and reflection
files, compiles on a miss, publishes each file independently, and performs
per-virtual-shader retention.

The generic `DerivedDataCache` module already exposes synchronous bounded
`bucket + key -> opaque immutable bytes` Get, Put, and Trim operations plus the
family-neutral Build Framework. Its filesystem backend atomically publishes one
`.bin` object, but a process-global mutex currently serializes every cache
operation. Shader compilation already owns worker execution, single-flight
coalescing, memory caching, source dependency resolution, compilation, and
typed publication; those responsibilities do not move into DDC.

This bounded plan is sufficient and does not require a roadmap. Remote/shared
backends, cooked Shader libraries, and removal of authoring compilation from
DurinGame are separate outcomes with independent acceptance gates.
...
- [ ] Inventory mounted-file and generated-source request flows, dependency
...
- [ ] Confirm the selected RenderCore private dependency and DurinGame DDC
...
- [ ] Replace the process-wide I/O mutex with bounded bucket synchronization:
  parallel ordinary operations and bucket-exclusive Trim, with lock metadata
  lifetime that cannot grow without bound.
- [ ] Preserve contained-path checks, two-character sharding, `.bin` objects,
...
- [ ] Avoid copying an immutable cache hit before a Shader-owned decoder can
...
- [ ] Add focused concurrency coverage for unrelated buckets/keys, concurrent
...
- [ ] Prove existing asset Build session policy, cache corruption recovery,
...
- [ ] Split `FShaderCacheStore` so dependency-manifest load/save and local
  fingerprint reuse have one explicitly machine-local owner independent of
  compiled-output persistence.
- [ ] Derive sorted virtual dependency identities from registered mounts while
  retaining physical fingerprints only inside the local manifest and resolver.
- [ ] Build source-tree and variant identities from virtual paths and content
...
- [ ] Preserve warm manifest validation without source-content reads when
...
- [ ] Add fixtures that mount byte-identical Shader closures beneath different
...
- [ ] Add Shader-private canonical encode/decode and validation for the complete
...
- [ ] Validate every Stage 0 format and request invariant before allocating or
  publishing a candidate; failure leaves the caller output empty and bounded.
- [ ] Add golden single-stage and multi-stage payloads plus truncated,
...
- [ ] Prove a complete compiler output round-trips without JSON, physical
...
- [ ] Replace disk-artifact lookup with bounded DDC Get followed by
...
- [ ] Cover mounted and generated sources, exact multi-entry-point requests,
...
- [ ] Add the private RenderCore dependency, qualify DurinEditor and DurinGame
...
- [ ] Remove compiled-output methods and retention state from the old
...
- [ ] Remove variant-directory, `.spv`, `.reflect.json`, binary/reflection path,
...
- [ ] Prove no production compiled Shader path calls `FShaderPaths` for a DDC
  object, parses reflection JSON, enumerates variant directories, or bypasses
  the generic DDC Cache API.
- [ ] Update the lasting Shader cache, asset-data lifecycle, code-module,
...
- [ ] Run the repository-prescribed focused DDC, Shader cache/contract,
...
- [`ShaderCacheStore.h`](../../Engine/Source/Runtime/RenderCore/Private/Shader/ShaderCacheStore.h)
- [`ShaderCacheStore.cpp`](../../Engine/Source/Runtime/RenderCore/Private/Shader/ShaderCacheStore.cpp)
...
- [`ShaderCacheStoreTests.cpp`](../../Engine/Tests/Native/RenderCoreTests/Private/ShaderCacheStoreTests.cpp)
