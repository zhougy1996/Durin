...
| Active implementation plan | Humans run `.\Documentation\DocTool.bat` for the interactive shell; agents run `.\Documentation\DocTool.bat list` for the compact Markdown index, then open only the matching plan |
| Completed plans awaiting monthly archive | Run `.\Documentation\DocTool.bat list --scope completed` |
| Named historical plan or required provenance | Run `.\Documentation\DocTool.bat list --scope archive --query "<title-or-filename>"`, then open only the selected archived plan |
