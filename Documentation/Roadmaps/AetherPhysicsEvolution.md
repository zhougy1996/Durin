...
architecture is now active through the
[Aether Geometry And Narrowphase Plan](../Plans/AetherGeometryAndNarrowphase.md).
It preserves the indexed scene boundary and M0 oracle while freezing immutable
shared geometry, compounds, complete primitive dispatch, contact/convergence
semantics, and a production replacement for the measured Capsule/Box nested
search. M3 remains gated on the completed M2 resource and dispatch boundary.
...
| M2: Geometry and narrow-phase architecture | Required; active 2026-08-12 | [Aether Geometry And Narrowphase](../Plans/AetherGeometryAndNarrowphase.md) | M1 body/index ownership and query pipeline | Immutable shared geometry references, compound simple shapes, operation/pair dispatch, complete primitive query matrix, analytic common fast paths, generic convex fallback, bounded penetration and shape casts, and pair/iteration diagnostics | Met for activation: existing transform/scale and tolerance semantics plus M0 result fields/ordering are frozen; the child plan freezes resource lifetime, compound precedence, convergence caps, and pair budgets before production dispatch changes | Qualified primitive and compound pairs match reference/goldens; Capsule movement removes pathological nested search from the production path; pair dispatch adds a new geometry type without editing scene traversal; shared geometry is not copied per body |
...
### [Aether Geometry And Narrowphase](../Plans/AetherGeometryAndNarrowphase.md)
