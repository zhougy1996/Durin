...
never submit one another directly. Payloads carry their own reserved sync point,
recorded storage and timing-query references, so a later reservation cannot
change an earlier payload's completion identity. Logical submission sync points
bind once to the recording payload's private reservation, never the queue's latest reservation, including when multiple
contexts interleave on one queue. Payloads retain every coalesced logical object;
callers keep their original references without resolving another handle.
Discarding an unsubmitted payload cancels its sync point and resets its command buffers; ambiguous native
...
`CanSubmitBatch` only observes admission; it never marks sync points submitted.
...
earlier payloads were accepted, earlier submissions on other queues retain their normal completion ownership.
The failed queue closes admission and quarantines both its accepted prefix and
the ambiguous call until teardown; remaining unsubmitted payloads are discarded.
Retained metadata stays Failed/DeviceLost rather than becoming successful. Batch validation does not roll back replayed
...
Cross-queue waits retain the producer sync point for batch ordering and native
...
provisioning can add an independent compute queue. Backend payload creation
reserves a monotonically increasing completion token; logical signal creation
during command recording reserves none. Successful `vkQueueSubmit` associates
the payload with one pooled fence and marks its logical sync points Submitted.
Polling or an exact
wait processes required timing results before publishing logical success and
advancing a contiguous completed-token watermark; it cannot skip an older
...
Exact waits route by queue ID and verify sync point ownership, including rejection
...
timeline semaphore and signals its private reservation value at native
submission. Cross-queue payload waits require accepted producer sync points
from this device; every success
...
owned by the completion tracker until their sync point retires. Native buffers,
...
conservatively captures the last reserved sync point from every physical queue;
...
uses the range's queue-qualified sync point to locate and submit its producer if
necessary, then waits the actual payload owners through the coordinator and
retries. The sync point is a producer locator, not an independent reuse authority.
...
readback range, submits its producing payload through the coordinator, waits its exact sync point,
...
The pool stores no sync point set or completion watermark. If both batches remain
busy, the coordinator finds the actual submissions holding the oldest retired
batch and waits their queue-qualified sync points. Unsubmitted external owners are
...
Outstanding sync point metadata becomes terminal failure, never synthetic success.
