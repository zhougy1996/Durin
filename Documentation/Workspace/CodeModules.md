...
| `TextureBuild` | Texture2D/TextureCube/VolumeTexture keys, normalized value recipes, private codecs/functions/helpers, one module-owned Build Function transaction, offline compression, diagnostics, and three typed synchronous value providers; no live Texture object, PostLoad, scheduler, or publication authority | [source](../../Engine/Source/Developer/TextureBuild) |
...
| local asset DDC request flow for StaticMesh, Texture2D/TextureCube/VolumeTexture, skeletal/animation, or Terrain | `DerivedDataCache` | `StaticMeshBuild`, `SkeletalBuild`, `TextureBuild`, or `TerrainBuild` for function inputs, recipe execution, payload validation, and typed result reconstruction; `Engine` owns every Texture object operation and publication |
