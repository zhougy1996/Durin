...
| `AssetTools` | Reusable editor policy and orchestration for package-backed creation, import, duplicate, save/resave, relocation, and redirector fix-up; owns relocation/fix-up workflows and staging, deletion operations, confirmed selection/reference policy, and deletion companion providers over Engine package mechanisms | [source](../../Engine/Source/Editor/AssetTools) |
...
| `AssetForgeBuiltins` | Direct built-in texture, static-mesh, Scene, and material import implementations | [source](../../Engine/Source/Editor/AssetForgeBuiltins) |
...
| `TextureBuild` | Pure Texture2D/TextureCube/VolumeTexture normalized-value recipes, panorama normalization, offline compression, recipe metrics and versions through one retained module interface; no DDC, Build Framework, key, payload codec, live Texture object, PostLoad, scheduler, or result-application authority | [source](../../Engine/Source/Developer/TextureBuild) |
| `MeshBuilder` | CPU StaticMesh render construction via FStaticMeshBuilder and IMeshBuilderModule, with retained module sessions and a render builder version; Engine owns keys, caching, PostLoad, and application | [source](../../Engine/Source/Developer/MeshBuilder) |
...
contract, reflected positional overrides, and shared material-binding updates. Render-state recreate
