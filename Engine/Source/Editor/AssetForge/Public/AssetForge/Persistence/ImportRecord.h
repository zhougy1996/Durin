...
{
	class DImportRecord;
	ASSETFORGE_API auto CreateImportRecordAsset(
	...
	class DImportRecord : public DObject
	{
		GENERATED_BODY()
	public:
		ASSETFORGE_API explicit DImportRecord(
		...
		ASSETFORGE_API auto GetState() const -> FImportRecordState;
		ASSETFORGE_API auto SetState(FImportRecordState State, std::string& OutError) -> bool;
		ASSETFORGE_API auto SetRecordIdForClone(const FGuid& NewId, std::string& OutError) -> bool;
		ASSETFORGE_API auto ReplaceOutputPath(
			const FAssetPath& OldPath,
			const FAssetPath& NewPath) -> bool;
		ASSETFORGE_API auto Validate(std::string& OutError) const -> bool;
		ASSETFORGE_API auto GetFingerprint() const -> std::string;
		ASSETFORGE_API auto ExchangeImportedState(DImportRecord& Other) noexcept -> void;
		ASSETFORGE_API auto PostLoad(std::string& OutError) -> bool override;

	private:
		friend ASSETFORGE_API auto CreateImportRecordAsset(
			const FAssetPath& Path,
			DImportRecord*& OutRecord
		) -> Asset::FAssetResult;
	...
	ASSETFORGE_API auto MakeImportRecordPayload(
		std::string SchemaId,
	...
	ASSETFORGE_API auto MakeSiblingImportRecordPath(
...
{
	template<>
	struct TDStructOpsTraits<AssetForge::FImportRecordOutput>
		: TDStructOpsTraitsBase<AssetForge::FImportRecordOutput>
	{
		static constexpr bool bWithPostDeserialize = true;

		static auto PostDeserialize(
			AssetForge::FImportRecordOutput& Value,
	...
	template<>
	struct TDStructOpsTraits<AssetForge::FImportRecordDetachedTombstone>
		: TDStructOpsTraitsBase<AssetForge::FImportRecordDetachedTombstone>
	{
		static constexpr bool bWithPostDeserialize = true;

		static auto PostDeserialize(
			AssetForge::FImportRecordDetachedTombstone& Value,
			FDStructPostDeserializeContext& Context) -> bool
		{
			std::string Error;
