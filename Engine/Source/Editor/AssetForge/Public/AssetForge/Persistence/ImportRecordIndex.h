...
namespace Durin::AssetForge
{
	...
	class ASSETFORGE_API FImportRecordIndex final
	...
	ASSETFORGE_API auto GetImportRecordIndex() -> FImportRecordIndex&;
	ASSETFORGE_API auto ComputeImportPackageFingerprint(
		DPackage* Package,
		std::string& OutFingerprint,
		std::string& OutError) -> bool;
	ASSETFORGE_API auto ComputePersistedImportPackageFingerprint(
		const FAssetPath& Path,
		std::string& OutFingerprint,
		std::string& OutError) -> bool;
	ASSETFORGE_API auto InspectImportRecordForOutput(
		const FAssetPath& OutputPath,
		FImportRecordIndex& Index) -> FImportRecordInspection;
	ASSETFORGE_API auto InspectImportRecord(
		const FAssetPath& RecordPath,
		FImportRecordIndex& Index) -> FImportRecordInspection;
	ASSETFORGE_API auto DetachImportRecordOutput(
	...
	ASSETFORGE_API auto RepairDuplicatedImportRecord(
		DImportRecord& Record,
		FImportRecordIndex& Index,
		const Asset::FAssetBundleSaveOptions& SaveOptions = {}) -> FImportRecordEditResult;
}
