...
{
	auto ReconcileImportedSurfaceOverrides(const FMaterialImportProvenance& Previous,
		...
		std::vector<std::string>& OutDiagnostics) -> bool
	{
		...
		std::vector<FGuid> UserOwners;
		OutDiagnostics.clear();
		const auto SameValue = [](const FMaterialParameterOverride& A, EMaterialParameterType Type,
			const FMaterialParameterValue& Value) { return A.Type == Type && A.Value == Value; };
		for (const auto& Parameter : Incoming.Parameters)
		{
			const auto Existing = std::ranges::find(Result, Parameter.OwnerId, &FMaterialParameterOverride::ParameterId);
			if (Existing != Result.end())
			{
				if (SameValue(*Existing, Parameter.Type, Parameter.Value)) continue;
				OutDiagnostics.push_back("Incoming logical roles disagree on a shared parameter owner.");
				return false;
			}
			Result.push_back({Parameter.OwnerId, Parameter.Type, Parameter.Value});
		}
		const auto KeepUserValue = [&](const FGuid& Owner, const FMaterialParameterOverride& Value) {
			const auto Existing = std::ranges::find(Result, Owner, &FMaterialParameterOverride::ParameterId);
			if (Existing == Result.end()) Result.push_back({Owner, Value.Type, Value.Value});
			else
			{
				if (std::ranges::find(UserOwners, Owner) != UserOwners.end() &&
					!SameValue(*Existing, Value.Type, Value.Value))
				{
					OutDiagnostics.push_back("Structural parent transition would merge conflicting user overrides.");
					return false;
				}
				*Existing = {Owner, Value.Type, Value.Value};
			}
			UserOwners.push_back(Owner);
			return true;
		};
		for (const auto& Local : LocalOverrides)
		{
			bool bKnown = false;
			for (const auto& Old : Previous.Parameters)
			{
				if (Old.OwnerId != Local.ParameterId) continue;
				bKnown = true;
				if (SameValue(Local, Old.Type, Old.Value)) continue;
				const auto Next = std::ranges::find(Incoming.Parameters, Old.LogicalId, &FMaterialImportedParameter::LogicalId);
				if (Next == Incoming.Parameters.end())
				{
					// A formerly shared owner may still exist for another role. Preserve
					// the disappearing role under its own stable logical identity.
					if (!KeepUserValue(Old.LogicalId, Local)) return false;
					OutDiagnostics.push_back("A user override whose imported role disappeared was retained as an orphan.");
				}
				else
				{
					if (Next->Type != Local.Type)
					{
						OutDiagnostics.push_back("An imported role changed type while retaining a user override.");
						return false;
					}
					if (!KeepUserValue(Next->OwnerId, Local)) return false;
				}
			}
			if (!bKnown)
			{
				if (!KeepUserValue(Local.ParameterId, Local)) return false;
				OutDiagnostics.push_back("An override outside the previous import receipt was preserved.");
			}
		}
		OutOverrides = std::move(Result);
		return true;
	}
