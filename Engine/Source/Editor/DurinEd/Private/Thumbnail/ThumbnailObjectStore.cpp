...
namespace Durin::Editor
{
	...
	struct FThumbnailObjectStore::FImpl
	{
		explicit FImpl(FAssetThumbnailPoolStorageSettings InSettings)
	...
	FThumbnailObjectStore::FThumbnailObjectStore(FAssetThumbnailPoolStorageSettings Settings)
	...
	{
		OutBytes.clear();
		if (!IsSafeKey(Key)) return EThumbnailObjectLoadResult::Invalid;
		FObjectIndexEntry Entry;
		{
			std::lock_guard Lock(Impl->Mutex);
			const auto It = Impl->Entries.find(std::string(Key));
			if (It == Impl->Entries.end()) return EThumbnailObjectLoadResult::Miss;
			Entry = It->second;
		}
	...
	auto FThumbnailObjectStore::Invalidate(std::string_view Key) -> void
	{
	...
	auto FThumbnailObjectStore::GetStats() const -> FThumbnailObjectStoreStats
	{
		std::lock_guard Lock(Impl->Mutex);
		return Impl->Stats;
	}

	auto SelectThumbnailBudgetEvictions(
		std::span<const FThumbnailBudgetEntry> Entries, uint64 BudgetBytes) -> std::vector<std::string>
	{
		uint64 ResidentBytes = 0;
		for (const FThumbnailBudgetEntry& Entry : Entries) ResidentBytes += Entry.Bytes;
		std::vector<const FThumbnailBudgetEntry*> Candidates;
		for (const FThumbnailBudgetEntry& Entry : Entries)
			if (Entry.Bytes > 0 && !Entry.bPinned) Candidates.push_back(&Entry);
		std::ranges::sort(Candidates, [](const FThumbnailBudgetEntry* A, const FThumbnailBudgetEntry* B) {
			return std::pair(A->LastUsed, A->Key) < std::pair(B->LastUsed, B->Key);
		});
		std::vector<std::string> Evictions;
		for (const FThumbnailBudgetEntry* Candidate : Candidates)
		{
