...
namespace Durin::Editor
{
	namespace
	{
		...
		auto BuildView(
			...
			const FThumbnailPreviewView& Preview
		) -> FSceneView
		{
			FSceneView View;
			const FVector3 Eye(
			...
			View.Settings.Mode.LODMode = Preview.bForceLOD0 ? EViewLODMode::ForceLOD0 : EViewLODMode::Automatic;
			View.ClearColor = {
				Preview.ClearRed,
				Preview.ClearGreen,
				Preview.ClearBlue,
		...
		auto MakeDefaultPreviewView() -> FThumbnailPreviewView
		{
			const FThumbnailVisualContract Contract;
	...
	struct FThumbnailPreviewScenePool::FImpl
	{
		struct FCapture
		{
			std::mutex Mutex;
		...
		FThumbnailVisualContract Contract;
		...
		FImpl(
			FThumbnailVisualContract InContract,
	...
	FThumbnailPreviewScenePool::FThumbnailPreviewScenePool(
		FAssetThumbnailOutputSettings Output,
	...
	FThumbnailPreviewScenePool::~FThumbnailPreviewScenePool() = default;

	auto FThumbnailPreviewScenePool::IsAvailable() const -> bool
	{
		return Impl->Error.empty() && Impl->PreviewScene != nullptr
			   && Impl->PreviewScene->IsAvailable() && Impl->RenderTarget != nullptr;
	}

	auto FThumbnailPreviewScenePool::GetDiagnostic() const -> std::string
	{
		return Impl->Error;
	}

	auto FThumbnailPreviewScenePool::GetWorld() -> DWorld*
	{
