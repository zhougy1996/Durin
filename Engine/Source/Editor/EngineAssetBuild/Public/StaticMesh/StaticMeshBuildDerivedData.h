...
namespace Durin::AssetBuild
{
	...
	ENGINEASSETBUILD_API auto BuildStaticMeshDerivedDataKeyBytes(
		const FStaticMeshBuildKeyInput& Input,
		std::string& OutError) -> std::vector<uint8>;
	ENGINEASSETBUILD_API auto BuildStaticMeshDerivedDataKey(
		const FStaticMeshBuildKeyInput& Input,
		std::string& OutError) -> std::string;
	ENGINEASSETBUILD_API auto BuildStaticMeshCollisionDerivedDataKeyBytes(
		const FStaticMeshCollisionBuildKeyInput& Input,
		std::string& OutError) -> std::vector<uint8>;
	ENGINEASSETBUILD_API auto BuildStaticMeshCollisionDerivedDataKey(
		const FStaticMeshCollisionBuildKeyInput& Input,
		std::string& OutError) -> std::string;
}
