...
namespace Durin::AssetBuild
{
	...
	ENGINEASSETBUILD_API auto SubmitTexture2DBuild(
		DTexture2D& Texture,
		FTexture2DAuthoringRequest Request,
		std::string& OutError,
		FTexture2DAuthoringCompletion Completion = {}) -> bool;
	ENGINEASSETBUILD_API auto GetTexture2DBuildDiagnostic(const DTexture2D& Texture)
		-> FTexture2DBuildDiagnostic;
	ENGINEASSETBUILD_API auto HasPendingTexture2DBuild(const DTexture2D& Texture) -> bool;
	ENGINEASSETBUILD_API auto CancelTexture2DBuild(DTexture2D& Texture) -> bool;
	ENGINEASSETBUILD_API auto WaitForTexture2DBuild(
		DTexture2D& Texture,
