...
namespace Durin::AssetBuild
{
	...
	ENGINEASSETBUILD_API auto BuildTexture2D(
	...
	ENGINEASSETBUILD_API auto PublishTexture2DProduct(
		DTexture2D& Texture,
	...
	ENGINEASSETBUILD_API auto MakeTexture2DDerivedDataKey(
		const DTexture2D& Texture,
		std::string& OutKey,
		std::string& OutError) -> bool;
	ENGINEASSETBUILD_API auto LoadTexture2DDerivedData(
		std::string_view Key,
