	...
	namespace
	{
		...
		public:
				...
				FEditorVisualizationCollector& Collector) const -> void override
			{
				...
				constexpr uint32 SegmentCount = 16;
				auto AddLine = [&](const FVector3& Start, const FVector3& End, int32 Priority = 80) {
					Collector.AddLine({Start, End, Color, 2.5f, 7.0f, Priority, PlayerStart, Root,
						EViewOverlayLinePattern::Solid, 12.0f, HoverColor});
				};

				for (uint32 Index = 0; Index < SegmentCount; ++Index)
				{
					const double AngleA = Math::TwoPi<double>() * static_cast<double>(Index) / SegmentCount;
					const double AngleB = Math::TwoPi<double>() * static_cast<double>(Index + 1) / SegmentCount;
				...
				const FVector3 ArrowStart = Origin;
				const FVector3 ArrowTip = Origin + Forward * 1.2;
				const FVector3 ArrowHead = ArrowTip - Forward * 0.3;
				AddLine(ArrowStart, ArrowTip, 90);
				AddLine(ArrowTip, ArrowHead + Right * 0.18, 90);
				AddLine(ArrowTip, ArrowHead - Right * 0.18, 90);
				AddLine(ArrowTip, ArrowHead + Up * 0.18, 90);
				AddLine(ArrowTip, ArrowHead - Up * 0.18, 90);
				Collector.AddBox({Origin, Color, MonaImGui::ScaleUI(Context.bSelected ? 14.0f : 12.0f),
					MonaImGui::ScaleUI(5.0f), 100, PlayerStart, Root, true, HoverColor});
			}
		};
	} // namespace
