...
namespace Durin::Editor::Material
{
		...
		FMaterialGraphClipboardPayload& OutPayload) const -> FMaterialGraphCommandResult
	{
		...
		if (NodeIds.empty() || NodeIds.size() > MaterialProgramMaxNodeCount)
			return RejectCommand("The material graph copy selection is empty or exceeds the node bound.");
		std::unordered_set<FGuid> Selected(NodeIds.begin(), NodeIds.end());
		if (Selected.size() != NodeIds.size()) return RejectCommand("The material graph copy selection contains duplicate node GUIDs.");
		for (const auto& Expression : Expressions)
			if (Cast<DMaterialExpressionMaterialOutput>(Expression.Get())) Selected.erase(Expression->Id);
		if (Selected.empty()) return RejectCommand("The material output terminal cannot be copied.");
		...
		for (const auto& Id : Selected)
		{
			if (std::ranges::none_of(Expressions, [&](const auto& Expression) { return Expression->Id == Id; }))
				return RejectCommand("A copied material graph node does not exist.");
			const auto It = Positions.find(Id);
			if (It == Positions.end()) return RejectCommand("A copied material graph node has no authored position.");
		...
		int32 X, int32 Y, DTransactor* Transactions) const -> FMaterialGraphCommandResult
	{
		...
		if (Payload.SchemaVersion != CurrentMaterialGraphClipboardSchemaVersion)
			return RejectCommand("The material graph clipboard schema version is unsupported.");
		...
		for (const auto& Node : Payload.Nodes)
		{
			if (!Node.Expression || !Node.Expression->Id.IsValid() || Remap.contains(Node.Expression->Id))
				return RejectCommand("The material graph clipboard contains an invalid or duplicate node GUID.");
			if (Node.RelativeX < 0 || Node.RelativeY < 0 || Node.RelativeX > MaterialGraphPresentationCoordinateLimit * 2
				...
				return RejectCommand("The material graph clipboard contains an invalid relative position.");
			FGuid Id;
			...
			if (const auto* Parameter = Cast<DMaterialExpressionParameter>(Node.Expression.Get()))
				...
				else if (*Existing != Definition) return RejectCommand("Clipboard shared parameter definitions disagree.");
			}
		}
		...
		for (bool bOutput : {false, true})
		{
			...
			for (const auto& Node : Payload.Nodes)
			{
				...
				if (!OldId.IsValid() || PortRemap.contains(OldId)) return RejectCommand("The clipboard contains duplicate or invalid function ports.");
		...
		for (auto& [OldId, Port] : PortRemap)
			if (Port.Default.Kind == EMaterialFunctionDefaultKind::Input)
			{
				...
				else if (!bSameRoot) return RejectCommand("A copied port default references an input outside the selection.");
			}
		const auto RemapLink = [&](FMaterialExpressionInput& Input) -> bool {
		...
		for (const auto& Entry : Payload.Nodes)
		{
			...
			VisitMaterialExpressionInputs(*Expression, [&](uint32, FMaterialExpressionInput& Input) {
				if (!RemapLink(Input)) bLinksValid = false;
			});
			if (!bLinksValid) return RejectCommand("The material graph clipboard references an unavailable external input.");
			...
			if (PositionX < -MaterialGraphPresentationCoordinateLimit || PositionX > MaterialGraphPresentationCoordinateLimit
				...
				return RejectCommand("Pasting would place a material graph node outside the supported coordinate range.");
