...
namespace Durin::Editor::SkeletalMesh
{
	namespace
	{
		...
		class FSession final : public ::Durin::Editor::IThumbnailRendererSession
		{
		public:
			explicit FSession(FSkeletalMeshThumbnailGenerationInput InInput)
			...
			{
				if (!Mesh) return {.State = ::Durin::Editor::EThumbnailRendererSessionState::Failed,
				...
				{
					AssetRevision = Status.Revision;
					return {.State = ::Durin::Editor::EThumbnailRendererSessionState::ReadyToRender,
			...
			auto PreparePreview(::Durin::Editor::IThumbnailPreviewScene& Scene,
		...
		private:
			FSkeletalMeshThumbnailGenerationInput Input;
	...
	auto DSkeletalMeshThumbnailRenderer::GetRegistration() const
		-> ::Durin::Editor::FThumbnailRenderingInfo
	{
		return {.AssetClassName = DSkeletalMesh::StaticClass()->GetQualifiedName().ToString(),
			.RendererName = std::string(FSkeletalMeshThumbnailContract::RendererName),
			.GeneratorSchemaVersion = FSkeletalMeshThumbnailContract::GeneratorSchemaVersion};
	}

	auto DSkeletalMeshThumbnailRenderer::CaptureGenerationRequest(
		...
		::Durin::Editor::FAssetThumbnailGenerationRequest& OutRequest, std::string& OutError) -> bool
	{
		...
		OutRequest.KeyInput = {.Output = Visual.Output,
			.PreviewFixtureIdentity = std::string(FSkeletalMeshThumbnailContract::PreviewFixtureIdentity),
			.PreviewFixtureVersion = FSkeletalMeshThumbnailContract::PreviewFixtureVersion,
	...
	auto DSkeletalMeshThumbnailRenderer::CreateGenerationSession(
		const ::Durin::Editor::FAssetThumbnailGenerationRequest&, const ::Durin::Editor::IAssetThumbnailGenerationInput& Input,
		std::string& OutError) -> std::unique_ptr<::Durin::Editor::IThumbnailRendererSession>
	{
		const auto* Typed = dynamic_cast<const FSkeletalMeshThumbnailGenerationInput*>(&Input);
