...
namespace Durin::Editor::Texture
{
		...
		-> ::Durin::Editor::EDocumentOpenResult
	{
		...
		FObjectPath Path;
		std::string Error;
		if (const auto PathValidation = FObjectPath::TryCreateWithDiagnostic(Document.ResourceId, Path); !PathValidation)
		{
		...
		DVolumeTexture* Texture) -> void
	{
		...
		const uint64 SelectionKey = static_cast<uint64>(State.Mip)
			...
			| (bPlatform ? 1ull << 60 : 0);
		if ((bPlatform || bSource) && (State.PlatformInput.lock() != Texture->GetPlatformDataShared() || State.SourceIdentity != Texture->GetSource().GetIdentity()
