...
def prepare_registered_project_config_file_paths() -> None:
    ...
    if DOGE_PROJECT_REGISTER_FILE_PATH.exists():
        ...
        for project_name, project_path in registered_projects_data["Projects"].items():
            REGISTERED_DOGE_PROJECTS[project_name] = Path(project_path)

def is_project_config_registered(project_name: str) -> bool:
    return project_name in REGISTERED_DOGE_PROJECTS

def is_project_config_loaded(project_name: str) -> bool:
    return project_name in PROJECT_CONFIGS

def _load_project_config_file(project_config_file_path: Path) -> DogeProjectConfig:
...
def _load_project_config(project_name: str) -> DogeProjectConfig:
    if project_name in REGISTERED_DOGE_PROJECTS:
        return _load_project_config_file(REGISTERED_DOGE_PROJECTS[project_name])
