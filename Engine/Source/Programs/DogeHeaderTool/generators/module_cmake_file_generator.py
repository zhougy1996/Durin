...
def _append_generated_files_to_cmake_content(content: list[str], module_name: str) -> None:
    module_config = configs.get_module_config(module_name)
    dht_output_dir = utils.get_module_dht_output_dir(module_name)
    generated_file_paths = []
    for header in module_config.reflect_headers:
        header_filename = Path(header).stem
        generated_file_paths.append(dht_output_dir / f"{header_filename}.gen.h")
        generated_file_paths.append(dht_output_dir / f"{header_filename}.gen.cpp")
    content.append("# Generated reflection files for this module\n")
    content.append("set(module_generated_reflection_files\n")
    for gen_file in generated_file_paths:
        content.append(f"    \"{gen_file.as_posix()}\"\n")
    content.append(")\n\n")
...
def _make_module_cmake_file_content(module_name: str) -> str:
    ...
    _append_generated_files_to_cmake_content(content, module_name)
