...
durin_source_dir = os.path.abspath(os.path.join(dht_dir, "../.."))
durin_root_dir = os.path.abspath(os.path.join(durin_source_dir, "../.."))
clang_lib_dir = os.path.join(durin_source_dir, "ThirdParty/clang/bin")
clang_args = [
    "-x",
    "c++",
    "-std=c++20",
...
class DHTProperty:
    ...
    def append_param_declaration(self, builder) -> list:
        builder.append(f"\tstatic const DurinCodeGen::{self.property_param} NewProp_{self.name};\n")
        return [f"NewProp_{self.name}"]

    def append_param_definition(self, builder, class_meta) -> None:
        builder.append(f"const DurinCodeGen::{self.property_param} {class_meta.construct_statics}::NewProp_{self.name} = {{ \"{self.name}\", EPropertyFlags::None, 1, STRUCT_OFFSET_UINT16({class_meta.name}, {self.name})}};\n")
...
class DHTHeader:
    ...
    def __init__(self, filepath):
        ...
        self.fid = "FID_DURIN_" + module_meta.name + "_" + self.relative_path.replace("/", "_").replace(".", "_")
