...
import durin_project as dproject
import durin_globals as g
import durin_header_tool as header_tool

# Utility functions
def init_logging(level: str) -> None:
    os.makedirs(g.DURIN_ENGINE_LOG_DIR, exist_ok=True)
...
def setup_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="A tool for the build system of Durin Engine.")
    subparsers = parser.add_subparsers(dest="function", required=True)

    # Setup common arguments
    common_parser = argparse.ArgumentParser(add_help=False)
    common_parser.add_argument("-p", "--project_file", help="Specify the Durin project file (.dproject), default is the engine project file.", required=True)
