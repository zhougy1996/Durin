...
class DurinModuleConfig:
    ...
    def get_manifest_path(self) -> str:
        project : DurinProjectConfig = g.projects.get(self.owning_project)
        if not project:
...
# load .dproject file
def load_project_config(filepath) -> DurinProjectConfig:
    proj = DurinProjectConfig()
    proj.from_file(filepath)
    return proj

# load .dmodule file
def load_module_config(filepath: str) -> DurinModuleConfig:
    module = DurinModuleConfig()
...
def generate_module_cmake_file(project_cfg: DurinProjectConfig, module_name: str, output_path: str) -> None:
...
def get_all_dependent_modules(project_cfg: DurinProjectConfig, module_name: str) -> list[DurinModuleConfig]:
    visited = set()
    result = []
