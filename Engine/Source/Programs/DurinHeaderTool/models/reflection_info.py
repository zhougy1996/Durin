...
def _init_clang() -> None:
    clang_lib_dir = configs.base_config.DURIN_ROOT_DIR / "Engine" / "Source" / "ThirdParty" / "clang" / "bin"
...
def _clang_args(module_name: str, export_mode: bool) -> list[str]:
    ...
    args.append(f"-I{configs.base_config.DURIN_ROOT_DIR / 'Engine' / 'Source' / 'Runtime' / 'Core' / 'Public'}")
    args.append(f"-I{configs.base_config.DURIN_ROOT_DIR / 'Engine' / 'Source' / 'ThirdParty'}")
