...
class TestReflectionProperties:
    ...
    def test_deprecated_property_route_is_inferred_from_the_member_name(self):
        actor = next(info for info in self.header_info.classes if info.short_name == "ASampleActor")
        ...
        assert "FPropertyDeprecationParams" not in self.generated_cpp
        assert "WithDeprecation" not in self.generated_cpp

    @pytest.mark.parametrize(
        ("name", "annotation", "diagnostic"),
        [
            ("Old", "DPROPERTY, Deprecated", "must end in _DEPRECATED"),
            ("Old_DEPRECATED", 'DPROPERTY, Deprecated, MigratesTo = "Value"', "does not accept migration metadata"),
            ("Old_DEPRECATED", "DPROPERTY, Deprecated, Edit", "cannot be combined with Edit or Transient"),
        ],
    )
    ...
    def test_container_spelling_contract_and_alias_resolution(self):
        ...
        byte_buffer = _make_property_from_spelling("Data", "FByteBuffer", symbols)
        qualified_byte_buffer = _make_property_from_spelling("Data", "Durin::FByteBuffer", symbols)
        assert byte is not None and byte.kind == "Byte"
        assert blob is None
        assert byte_buffer is not None and byte_buffer.kind == "Blob" and byte_buffer.inner is None
        assert qualified_byte_buffer is not None and qualified_byte_buffer.kind == "Blob"
        bulk = _make_property_from_spelling(
            "Payload", "Durin::FEditorBulkData", symbols
        )
        ...
        assert _make_property_from_spelling(
            "NestedAlias", "std::vector<FByteBuffer>", symbols
        ) is None
