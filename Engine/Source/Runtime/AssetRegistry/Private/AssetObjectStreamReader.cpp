...
namespace Durin::Asset::PackageObjectStream
{
	namespace
	{
			...
			std::vector<FAssetReferenceEdge>& Out, FReaderDiagnostic& Diagnostic) -> bool
		{
			...
			{
				const std::string_view Target = Value.ReferenceTag == 1
					? SourcePackage.GetView()
					: std::string_view(Package.Header.Dependencies[
						static_cast<size_t>(Value.ReferenceId - 1)]);
