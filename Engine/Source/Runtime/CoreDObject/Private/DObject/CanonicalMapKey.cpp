...
namespace Durin::ObjectPackage
{
	namespace
	{
		...
		auto AppendBigEndian(std::vector<std::byte>& Out, T Value) -> void
		{
		...
		auto AppendSortableInteger(std::vector<std::byte>& Out, T Value) -> void
		{
		...
		template<std::unsigned_integral T>
		auto AppendSortableFloatBits(std::vector<std::byte>& Out, T Bits) -> void
		{
	...
	auto BuildCanonicalMapKeyToken(const FSerializedType& Type, const FSerializedValue& Value,
		std::vector<std::byte>& OutToken, std::string* OutError) -> bool
	{
