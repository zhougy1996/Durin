...
namespace Durin
{
	...
	namespace
	{
		struct FSerializedPropertyMoveRegistry
		{
			std::mutex Mutex;
			std::vector<FSerializedPropertyMove> Moves;
		};

		auto GetSerializedPropertyMoveRegistry() -> FSerializedPropertyMoveRegistry&
		{
			// Routes intentionally live for the process lifetime. This avoids static
			// destruction ordering between reflected modules and CoreDObject.
			static auto* Registry = new FSerializedPropertyMoveRegistry;
			return *Registry;
		}
	}

	auto RegisterSerializedPropertyMoves(
		...
		std::string* OutError) -> bool
	{
		...
		auto& Registry = GetSerializedPropertyMoveRegistry();
		std::scoped_lock Lock(Registry.Mutex);
		std::vector<FSerializedPropertyMove> Candidate = Registry.Moves;
		for (const FSerializedPropertyMove& Move : Moves)
		{
			if (Move.StoredDeclaringType.empty() || Move.StoredName.empty()
				|| Move.CurrentDeclaringType.empty() || Move.CurrentName.empty())
				return Fail("Serialized property move identities must not be empty.");
			if (Move.StoredDeclaringType == Move.CurrentDeclaringType
				&& Move.StoredName == Move.CurrentName)
				return Fail("Serialized property move must change the field identity.");
			const auto Existing = std::ranges::find_if(Candidate, [&](const auto& Item) {
				return Item.StoredDeclaringType == Move.StoredDeclaringType
					&& Item.StoredName == Move.StoredName;
			});
			if (Existing != Candidate.end())
			{
				if (*Existing != Move)
					return Fail(std::format(
						"Serialized property move {}::{} has conflicting destinations.",
						Move.StoredDeclaringType, Move.StoredName));
				continue;
			}
			Candidate.push_back(Move);
		}
		std::ranges::sort(Candidate, [](const auto& Left, const auto& Right) {
			return std::tie(Left.StoredDeclaringType, Left.StoredName,
				Left.CurrentDeclaringType, Left.CurrentName)
				< std::tie(Right.StoredDeclaringType, Right.StoredName,
					Right.CurrentDeclaringType, Right.CurrentName);
		});
		Registry.Moves = std::move(Candidate);
		if (OutError) OutError->clear();
		return true;
	}

	auto CaptureSerializedPropertyMoves() -> std::vector<FSerializedPropertyMove>
	{
		auto& Registry = GetSerializedPropertyMoveRegistry();
		std::scoped_lock Lock(Registry.Mutex);
		return Registry.Moves;
	}
...
IMPLEMENT_INTRINSIC_CLASS(DType, DType, COREDOBJECT_API, DObject, DObject, COREDOBJECT_API, {})

IMPLEMENT_INTRINSIC_CLASS(DStructBase, DStructBase, COREDOBJECT_API, DType, DType, COREDOBJECT_API, {})

IMPLEMENT_INTRINSIC_CLASS(DClass, DClass, COREDOBJECT_API, DStructBase, DStructBase, COREDOBJECT_API, {})

IMPLEMENT_INTRINSIC_CLASS(DStruct, DStruct, COREDOBJECT_API, DStructBase, DStructBase, COREDOBJECT_API, {})

IMPLEMENT_INTRINSIC_CLASS(DEnum, DEnum, COREDOBJECT_API, DType, DType, COREDOBJECT_API, {})
