...
namespace Durin
{
	...
	{
		if (Summary.Flags != 0)
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::UnsupportedFlags, .Summary = Summary, .Actual = Summary.Flags});
		if (Summary.Extent > PackageBulkDataMaximumSegmentBytes)
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::SegmentLimit, .Summary = Summary, .Actual = Summary.Extent, .Expected = PackageBulkDataMaximumSegmentBytes});
		if ((Summary.Extent == 0) != Summary.Digest.IsZero())
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::DigestPresence, .Summary = Summary});
		if (Entries.size() > PackageBulkDataMaximumFieldCount)
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::FieldLimit, .Summary = Summary, .Actual = Entries.size(), .Expected = PackageBulkDataMaximumFieldCount});
		...
		for (size_t Index = 0; Index < Entries.size(); ++Index)
		{
			...
			if (Entry.FieldIndex != Index + 1)
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::NonCanonicalIndex, .Index = Index, .Summary = Summary, .Entry = Entry, .Actual = Entry.FieldIndex, .Expected = Index + 1});
			if (Entry.StorageFlags != 0 || Entry.StoredSize != Entry.LogicalSize)
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::UnsupportedStorage, .Index = Index, .Summary = Summary, .Entry = Entry});
			if (Entry.ContentId.IsZero())
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::MissingContentIdentity, .Index = Index, .Summary = Summary, .Entry = Entry});
			...
			if (Entry.Placement == EPackageBulkDataPlacement::Inline)
			{
				if (Entry.SegmentOffset != 0 || Entry.Alignment != 1
					...
					return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::InvalidInlineMetadata, .Index = Index, .Summary = Summary, .Entry = Entry});
				continue;
			}
			if (Entry.Placement != EPackageBulkDataPlacement::External)
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::InvalidPlacement, .Index = Index, .Summary = Summary, .Entry = Entry});
			...
			if (Entry.StoredSize <= EditorBulkDataExternalThreshold
				...
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::InvalidExternalMetadata, .Index = Index, .Summary = Summary, .Entry = Entry, .Actual = Entry.SegmentOffset, .Expected = ExpectedOffset});
			if (Entry.SegmentOffset > Summary.Extent
				|| Entry.StoredSize > Summary.Extent - Entry.SegmentOffset)
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::RangeOutsideSegment, .Index = Index, .Summary = Summary, .Entry = Entry});
		...
		if (bSawExternal ? PreviousExternalEnd != Summary.Extent : Summary.Extent != 0)
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::InvalidFinalExtent, .Summary = Summary, .Actual = Summary.Extent, .Expected = PreviousExternalEnd});
		return {};
	}
		...
		FByteView Segment) -> FPackageBulkDataResult
	{
		...
		if (Segment.size() != Summary.Extent)
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::ExtentMismatch, .Summary = Summary, .Actual = Segment.size(), .Expected = Summary.Extent});
		...
		if (Summary.Extent != 0 && ActualDigest != Summary.Digest)
			return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::SegmentDigestMismatch, .Summary = Summary, .ActualDigest = ActualDigest});
		...
		for (size_t Index = 0; Index < Entries.size(); ++Index)
		{
			...
			for (; Cursor < Entry.SegmentOffset; ++Cursor)
				if (Segment[static_cast<size_t>(Cursor)] != std::byte{0})
					return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::NonzeroPadding, .Index = Index, .Summary = Summary, .Entry = Entry, .Actual = std::to_integer<uint8>(Segment[static_cast<size_t>(Cursor)]), .Offset = Cursor});
			...
			if (ActualDigest != Entry.ContentId)
				return std::unexpected(FPackageBulkDataError{.Code = EPackageBulkDataError::FieldDigestMismatch, .Index = Index, .Summary = Summary, .Entry = Entry, .ActualDigest = ActualDigest});
