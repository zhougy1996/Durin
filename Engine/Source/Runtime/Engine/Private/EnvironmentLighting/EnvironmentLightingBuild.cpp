...
namespace Durin
{
	namespace
	{
		...
		auto Hammersley(uint32 Index, uint32 Count) -> glm::vec2
		{
			return {static_cast<float>(Index) / static_cast<float>(Count),
				RadicalInverse(Index)};
		}

		auto MakeBasisSample(const glm::vec3& Normal, const glm::vec3& Local) -> glm::vec3
		{
		...
		auto SampleStudioRadiance(const glm::vec3& Direction) -> glm::vec3
		{
			const float SkyAmount = std::clamp(Direction.z * 0.5f + 0.5f, 0.0f, 1.0f);
			glm::vec3 Radiance = glm::mix(
				glm::vec3(0.025f, 0.020f, 0.018f),
				glm::vec3(0.18f, 0.28f, 0.50f),
				SkyAmount);
			const glm::vec3 KeyDirection = glm::normalize(glm::vec3(-0.4f, 0.5f, 0.75f));
			const float Key = std::pow(
				std::clamp(glm::dot(Direction, KeyDirection), 0.0f, 1.0f), 256.0f);
			return Radiance + 6.0f * glm::vec3(1.0f, 0.78f, 0.55f) * Key;
		}
		...
		auto StoreRgba16(std::vector<uint16>& Pixels, const glm::vec3& Value) -> void
		{
		...
		auto BuildIrradianceFace(ETextureCubeFace Face) -> std::vector<uint16>
		{
			...
			for (uint32 Y = 0; Y < EnvironmentIrradianceDimension; ++Y)
			{
				for (uint32 X = 0; X < EnvironmentIrradianceDimension; ++X)
				{
					...
					const glm::vec3 Normal = glm::normalize(glm::vec3(PixelDirection));
					glm::vec3 Sum(0.0f);
					for (uint32 Sample = 0; Sample < IrradianceSampleCount; ++Sample)
					{
						const glm::vec2 Xi = Hammersley(Sample, IrradianceSampleCount);
						const float Phi = 2.0f * Pi * Xi.x;
						const float CosTheta = std::sqrt(1.0f - Xi.y);
						const float SinTheta = std::sqrt(Xi.y);
						const glm::vec3 Direction = MakeBasisSample(Normal, {
							std::cos(Phi) * SinTheta,
							std::sin(Phi) * SinTheta,
							CosTheta});
				...
				for (uint32 X = 0; X < Dimension; ++X)
				{
					...
					for (uint32 Sample = 0; Sample < SampleCount; ++Sample)
					{
						...
						const glm::vec3 Direction = glm::normalize(
							2.0f * glm::dot(Normal, Half) * Half - Normal);
						const float NoL = std::max(glm::dot(Normal, Direction), 0.0f);
						if (NoL > 0.0f)
						{
					...
					StoreRgba16(
						Pixels, Weight > 0.0f ? Sum / Weight : glm::vec3(0.0f));
				}
			}
			return Pixels;
		}
			...
			for (uint32 Y = FirstRow; Y < EndRow; ++Y)
			{
				...
				for (uint32 X = 0; X < EnvironmentBrdfLutDimension; ++X)
				{
					...
					for (uint32 Sample = 0; Sample < BrdfSampleCount; ++Sample)
					{
						...
						const float VoH = std::max(glm::dot(View, Half), 0.0f);
						const glm::vec3 Light = glm::normalize(2.0f * VoH * Half - View);
