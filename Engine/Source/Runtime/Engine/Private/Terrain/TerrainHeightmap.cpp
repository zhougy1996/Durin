...
namespace Durin
{
	...
	auto DTerrainHeightmap::ImportAsset(
		...
		bool bEngineAuthoringContext) -> FTerrainHeightmapImportResult
	{
		...
		if (!PrepareMountedSourceFile(
			Input, ParsedPath.ToString(), StoredSourcePath, MountedSource, Error,
			bEngineAuthoringContext))
			return {false, std::move(Error), nullptr};
		std::vector<uint8> EncodedBytes;
		if (!FFileHelper::LoadFileToArray(EncodedBytes, MountedSource.PhysicalPath.generic_string()))
		{
		...
		if (!Heightmap->BuildFromEncodedBytes(
			EncodedBytes, MountedSource.SourcePath, Error))
		{
			RollbackMountedSourceFile(MountedSource);
		...
		Heightmap->SourceFileSize = std::filesystem::file_size(
			MountedSource.PhysicalPath, FingerprintError);
		const auto LastWrite = std::filesystem::last_write_time(
			MountedSource.PhysicalPath, FingerprintError);
		Heightmap->SourceLastWriteTime = FingerprintError ? 0
		...
		(void)AssetPath;
		(void)Settings;
		(void)bEngineAuthoringContext;
		return {false, "Terrain heightmap import is unavailable in runtime-only builds.", nullptr};
#endif
	}
}
