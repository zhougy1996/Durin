...
namespace Durin
{
		...
		uint8 SourceChannelCount, uint8 TransparencyMask) -> std::optional<FTextureSource>
	{
		if (Mips.empty())
		{
			DURIN_WARN("Texture2D source mip chain is empty.");
			return std::nullopt;
		}
		...
		for (size_t Index = 0; Index < Mips.size(); ++Index)
		{
			...
			{
				DURIN_WARN("Texture2D source mip {} is non-canonical ({}x{}, depth {}, slices {}).",
					Index, Info.Width, Info.Height, Info.Depth, Info.SliceCount);
				return std::nullopt;
			}
		...
		if (Base.Format != Image::ERawImageFormat::RGBA8
			|| !NewSource.InitLayered(ETextureSourceKind::Texture2D,
