...
namespace Durin
{
	namespace
	{
		...
		auto EstimateBuildBytes(const FTexture2DBuildRequest& Request) -> uint64
		{
			...
			if (WorkingBytes == 0)
				WorkingBytes = std::max<uint64>(SaturatingMultiply(Request.EncodedSource.size(), 8), 64ull * 1024ull * 1024ull);
			return SaturatingAdd(Request.EncodedSource.size(), WorkingBytes);
		}
		...
		: public std::enable_shared_from_this<FTexture2DBuildCoordinator::FState>
	{
		...
		{
			if (!Completion || Request.AssetIdentity.empty() || Request.EncodedSource.empty()) return 0;
			...
			const FTaskCancellationToken& Token) -> void
		{
			...
			SetPhase(Job, ETexture2DBuildPhase::Decoding);
			if (Cancel())
			{
				Result.Phase = ETexture2DBuildPhase::Cancelled;
				Result.Error = "Texture build was cancelled.";
			...
			const uint64 DecodeStart = NowNanoseconds();
			auto SourceData = std::make_unique<FTextureSourceData>();
			if (!TextureBuild::DecodeRGBA8(Job->Request.EncodedSource, *SourceData, Result.Error))
			{
				Result.Metrics.DecodeNanoseconds = NowNanoseconds() - DecodeStart;
				...
				if (Result.Phase == ETexture2DBuildPhase::Failed)
					Result.FailurePhase = ETexture2DBuildPhase::Decoding;
				CompleteAdmitted(Job, std::move(Result));
				return;
			}
			Result.Metrics.DecodeNanoseconds = NowNanoseconds() - DecodeStart;
			Result.Metrics.DecodedBytes = SourceData->Pixels.size();
			Result.SourceHash = FXxHash128::HashBuffer(Job->Request.EncodedSource);
			if (Job->Request.bHasExpectedSourceHash
				&& (Result.SourceHash.HashLow != Job->Request.ExpectedSourceHash.HashLow
					|| Result.SourceHash.HashHigh != Job->Request.ExpectedSourceHash.HashHigh))
			{
				Result.Error = "Texture source bytes no longer match the submitted content identity.";
				Result.FailurePhase = ETexture2DBuildPhase::Decoding;
				CompleteAdmitted(Job, std::move(Result));
				return;
			}
			...
			std::string Error) -> void
		{
			...
			std::vector<uint8>().swap(Job->Request.EncodedSource);
