...
namespace Durin
{
	namespace
	{
		...
		constexpr std::string_view TextureCubeLegacySourceHandler =
			"Engine.TextureCube.LegacySourceFiles";
		constexpr std::string_view TextureCubeClassName = "Durin::DTextureCube";
		constexpr std::array<std::string_view, TextureCubeFaceCount + 1>
			LegacySourceFieldNames = {
				"PositiveXSourceFile",
				"NegativeXSourceFile",
				"PositiveYSourceFile",
				"NegativeYSourceFile",
		...
		struct FLegacyTextureCubeSources
		{
			std::array<const Asset::FAssetLegacyField*, TextureCubeFaceCount + 1> Fields{};
			std::array<std::string, TextureCubeFaceCount + 1> Paths;

			auto HasFields() const -> bool
			{
			...
			auto CollectFields() const -> std::vector<Asset::FAssetLegacyField>
			{
				std::vector<Asset::FAssetLegacyField> Result;
				for (const Asset::FAssetLegacyField* Field : Fields)
					if (Field) Result.push_back(*Field);
				return Result;
			}
		};

		auto ReadLegacyTextureCubeSources(
			std::span<const Asset::FAssetLegacyField> Fields,
			FLegacyTextureCubeSources& OutSources) -> Asset::FAssetResult
		{
			OutSources = {};
			for (size_t Index = 0; Index < LegacySourceFieldNames.size(); ++Index)
			{
				const auto It = std::ranges::find_if(
					Fields, [Index](const Asset::FAssetLegacyField& Field) {
						return Field.DeclaringClass == TextureCubeClassName
							&& Field.Name == LegacySourceFieldNames[Index]
							&& Field.Kind == DurinCodeGen::EPropertyGenFlags::String
							&& Field.TypeSignature == "String";
					});
				if (It == Fields.end()) continue;
				OutSources.Fields[Index] = &*It;
				if (!Asset::FAssetPackageField{
					.DeclaringClass = It->DeclaringClass,
					.Name = It->Name,
					.Kind = It->Kind,
					.TypeSignature = It->TypeSignature,
					.Payload = It->Payload}.TryReadString(OutSources.Paths[Index]))
					return {
						Asset::EAssetError::CorruptFile,
						std::format(
							"Invalid legacy TextureCube {} payload.",
							LegacySourceFieldNames[Index])};
			}
			return {};
		}

		auto ResolveLegacyCubeSource(
			...
			std::string& OutError) -> bool
		{
			...
			{
				OutError = "Legacy TextureCube source migration requires an owning package.";
				return false;
			}
			std::string SourcePath(LegacyPath);
			constexpr std::string_view SourceAssetsRoot = "SourceAssets";
			if (LegacyPath.starts_with(SourceAssetsRoot))
			{
				const PathUtilities::FMountPoint* Mount =
					FindOwningMount(Texture.GetPackage()->GetPackagePath());
				if (!Mount)
				{
					OutError = "Legacy TextureCube source migration could not resolve the package mount.";
					return false;
				}
				std::string_view Relative = LegacyPath.substr(SourceAssetsRoot.size());
				if (Relative.starts_with('/')) Relative.remove_prefix(1);
				SourcePath = Mount->VirtualRoot + std::string(Relative);
			}
			else if (!LegacyPath.starts_with('/'))
			{
				OutError =
		...
		const bool GTextureCubeLegacySourceInspectionUpgraderRegistered = [] {
			Asset::RegisterAssetStructureInspectionUpgrader(
				std::string(TextureCubeClassName),
				std::string(TextureCubeLegacySourceHandler),
				[](const Asset::FAssetPackageInspection&,
					...
					-> Asset::FAssetResult
				{
					FLegacyTextureCubeSources LegacySources;
					Asset::FAssetResult Result =
						ReadLegacyTextureCubeSources(Fields, LegacySources);
					if (!Result || !LegacySources.HasFields()) return Result;
					...
					const bool bHasCanonicalProvenance = ProvenanceField
						&& ProvenanceField->TryReadStruct(
							FTextureCubeSourceImportData::StaticStruct(), &Provenance)
						&& Provenance.HasSource();
					const bool bSafeCleanup =
						!LegacySources.HasPaths() || bHasCanonicalProvenance;
					OutIssues.push_back({
						.DeclaringClass = std::string(TextureCubeClassName),
						.LegacyFields = LegacySources.CollectFields(),
						...
						.MigrationSummary = bSafeCleanup
							? "Will remove retired TextureCube source-string fields; canonical provenance is already authoritative."
							: "TextureCube has only retired source-string metadata. Load the asset to attempt SourceAssets migration, or reimport it for manual repair.",
						.Risk = bSafeCleanup
					...
					-> Asset::FAssetResult
				{
					...
					if (!Texture)
						return {
							...
							"TextureCube legacy-source upgrader received an incompatible object."};
					FLegacyTextureCubeSources LegacySources;
					Asset::FAssetResult Result =
						ReadLegacyTextureCubeSources(Fields, LegacySources);
					if (!Result || !LegacySources.HasFields()) return Result;

					if (Texture->SourceImportData.HasSource()
						|| !LegacySources.HasPaths())
					{
						OutIssues.push_back({
							.DeclaringClass = std::string(TextureCubeClassName),
							.LegacyFields = LegacySources.CollectFields(),
							.Classification =
								Asset::EAssetCompatibilityClassification::SafeCleanup,
							.MigrationSummary =
								"Removed retired TextureCube source-string fields; canonical provenance remains authoritative.",
					...
					const size_t PanoramaIndex = TextureCubeFaceCount;
					const bool bCompleteLegacyLayout = bPanorama
						? !LegacySources.Paths[PanoramaIndex].empty()
							&& std::ranges::none_of(
								std::span(LegacySources.Paths).first(TextureCubeFaceCount),
								[](const std::string& Path) { return !Path.empty(); })
						: LegacySources.Paths[PanoramaIndex].empty()
							&& std::ranges::all_of(
								std::span(LegacySources.Paths).first(TextureCubeFaceCount),
								[](const std::string& Path) { return !Path.empty(); });
					if (!bCompleteLegacyLayout)
						return {
							Asset::EAssetError::UnsupportedProperty,
							"Legacy TextureCube source metadata is incomplete or conflicts with "
							"SourceLayout. Reimport the asset to create canonical SourceAssets provenance."};

					FTextureCubeSourceImportData Migrated{
						.SourceLayout = Texture->SourceLayout,
						.DecoderId = std::string(TextureDecoderId),
						.DecoderVersion = TextureDecoderVersion,
						.ProjectionVersion = TextureCubeProjectionVersion};
					std::string Error;
					const size_t SourceCount = bPanorama ? 1 : TextureCubeFaceCount;
					for (size_t SourceIndex = 0;
						SourceIndex < SourceCount;
						++SourceIndex)
					{
						const size_t LegacyIndex =
							bPanorama ? PanoramaIndex : SourceIndex;
						FMountedSourceFile Source;
						if (!ResolveLegacyCubeSource(
							*Texture, LegacySources.Paths[LegacyIndex], Source, Error))
							return {
								Asset::EAssetError::UnsupportedProperty,
								std::format(
									"Legacy TextureCube source '{}' requires manual repair: {}. "
									"Reimport the asset to create canonical SourceAssets provenance.",
									LegacySources.Paths[LegacyIndex], Error)};
						...
						if (!HashTextureSource(Source.PhysicalPath, SourceHash, Error))
							return {
								...
								std::format(
									"Legacy TextureCube source '{}' requires manual repair: {}.",
									LegacySources.Paths[LegacyIndex], Error)};
						FTextureSourceFile MigratedSource = {
							.SourcePath = Source.SourcePath,
							.SourceContentHashLow = SourceHash.HashLow,
							.SourceContentHashHigh = SourceHash.HashHigh};
						if (bPanorama)
							Migrated.Panorama = std::move(MigratedSource);
						else
							Migrated.GetMutableFace(
								static_cast<ETextureCubeFace>(SourceIndex)) =
								std::move(MigratedSource);
					}
					Texture->SourceImportData = std::move(Migrated);
					OutIssues.push_back({
						.DeclaringClass = std::string(TextureCubeClassName),
						.LegacyFields = LegacySources.CollectFields(),
						.Classification =
						...
						.MigrationSummary =
							"Migrated retired TextureCube source-string metadata to canonical SourceImportData provenance.",
						.MigratedDataCount = SourceCount,
						.Risk = Asset::EAssetCompatibilityRisk::None});
					return {};
				});
			return true;
