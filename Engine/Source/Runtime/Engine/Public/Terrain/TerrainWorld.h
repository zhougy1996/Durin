...
namespace Durin
{
	...
	struct FTerrainTileRecipeInput
	{
		FTerrainTileKey Tile;
	...
	ENGINE_API auto TerrainFloorDiv(int64 Value, int64 Divisor, int64& OutResult) -> bool;
	ENGINE_API auto TerrainFloorMod(int64 Value, int64 Divisor, int64& OutResult) -> bool;
	ENGINE_API auto ValidateTerrainWorldDefinition(
		const FTerrainWorldDefinition& Definition, ETerrainWorldOutcome& OutOutcome,
		std::string& OutError) -> bool;
	ENGINE_API auto ResolveTerrainSampleAddress(
		...
		ETerrainWorldOutcome& OutOutcome, std::string& OutError) -> bool;
	ENGINE_API auto GetTerrainTileSampleExtent(
		const FTerrainTileKey& Tile, FTerrainSampleExtent& OutExtent,
		ETerrainWorldOutcome& OutOutcome, std::string& OutError) -> bool;
	ENGINE_API auto TerrainSampleToWorldPosition(
		const FTerrainWorldCoordinates& Coordinates, FTerrainGlobalSample Sample,
		int16 Height, std::array<double, 3>& OutPosition,
		ETerrainWorldOutcome& OutOutcome, std::string& OutError) -> bool;
	ENGINE_API auto NormalizeTerrainHeightQuantum(
		int32 Height, int16& OutHeight, ETerrainWorldOutcome& OutOutcome,
		std::string& OutError) -> bool;
	ENGINE_API auto ValidateTerrainNormalizedTileInput(
		const FTerrainTileRecipeInput& Input, ETerrainWorldOutcome& OutOutcome,
		std::string& OutError) -> bool;
	ENGINE_API auto EncodeTerrainTileProduct(
		...
		ETerrainWorldOutcome& OutOutcome, std::string& OutError) -> bool;
	ENGINE_API auto DecodeTerrainTileProduct(
		...
		std::string& OutError) -> bool;
	ENGINE_API auto ValidateTerrainWorldProductBody(
		ETerrainTileProductClass ProductClass,
		std::span<const std::byte> Bytes, std::string& OutError) -> bool;
	ENGINE_API auto GetTerrainWorldProductBodyMaximumBytes(
		ETerrainTileProductClass ProductClass) -> uint64;
