...
namespace Durin::VulkanRHI
{
		...
		-> FVulkanPhysicalDeviceCandidateEvaluation
	{
		...
		if (Input.ApiVersion < VK_API_VERSION_1_1)
			Result.RejectionReasons.push_back(std::format("device API version is below Vulkan 1.1 actual={} required={}", Input.ApiVersion, VK_API_VERSION_1_1));
		...
		if (Input.bRequirePortabilitySubset
			...
			Result.RejectionReasons.emplace_back("missing platform required extension VK_KHR_portability_subset");
		...
		if (Input.MaxImageArrayLayers < TextureCubeFaceCount)
			Result.RejectionReasons.push_back(std::format("maxImageArrayLayers is below six actual={} required={}", Input.MaxImageArrayLayers, TextureCubeFaceCount));
		if (std::ranges::any_of(Input.MaxComputeWorkGroupCount,
			[](uint32 Limit) { return Limit == 0; }))
			Result.RejectionReasons.emplace_back("maxComputeWorkGroupCount contains a zero limit");
