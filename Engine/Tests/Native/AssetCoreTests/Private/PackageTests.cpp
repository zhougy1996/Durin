...
namespace
{
	...
	auto MakeSchemaTestClass(std::string_view QualifiedName, Durin::DClass* SuperClass = nullptr)
		-> Durin::DClass*
	{
		auto* Class = new Durin::DClass(
			Durin::EC_StaticConstructor,
			Durin::FName(QualifiedName),
			sizeof(DPackageAssetForTest),
			alignof(DPackageAssetForTest),
			Durin::EObjectFlags::NoFlags,
			Durin::EClassFlags::None,
			Durin::EClassCastFlags::DClass,
			(Durin::DClass::ClassConstructorType)Durin::InternalConstructor<DPackageAssetForTest>);
		Class->SetSuperStructBase(SuperClass ? SuperClass : Durin::DObject::StaticClass());
		return Class;
	}

	auto MakeSchemaMigrationStep(
		Durin::uint32 FromVersion,
		std::string HandlerId,
		Durin::Asset::EAssetCompatibilityRisk Risk =
			Durin::Asset::EAssetCompatibilityRisk::None)
		-> Durin::Asset::FAssetSchemaMigrationStep
	{
		const std::string Summary = std::format(
			"Apply schema migration {} -> {}.", FromVersion, FromVersion + 1);
		return {
			.FromVersion = FromVersion,
			.HandlerId = std::move(HandlerId),
			.Summary = Summary,
			.Risk = Risk,
			.Inspect = [](const Durin::Asset::FAssetPackageInspection&,
				const Durin::Asset::FAssetPackageObjectInspection&,
				std::span<const Durin::Asset::FAssetLegacyField>,
				std::vector<Durin::Asset::FAssetCompatibilityIssue>&)
				-> Durin::Asset::FAssetResult { return {}; },
			.Apply = [](Durin::DObject*,
				std::span<const Durin::Asset::FAssetLegacyField>,
	...
	auto MakeSchemaDescriptor(
		Durin::Asset::FAssetSchemaId Id,
		std::string Name,
		Durin::uint32 LatestVersion,
		Durin::uint32 MinimumReadableVersion,
		std::vector<Durin::DClass*> Classes,
		std::vector<Durin::Asset::FAssetSchemaMigrationStep> Steps)
		-> Durin::Asset::FAssetSchemaDescriptor
	{
		return {
			.Id = Id,
			.Name = std::move(Name),
			...
			.AffectedClasses = std::move(Classes),
			.MigrationSteps = std::move(Steps),
			.ValidateCurrent = [](const Durin::DObject*) -> Durin::Asset::FAssetResult {
				return {};
			}};
	}
...
TEST(FPackageAssetTests, CustomVersionsAreSortedAndMissingVersionsQueryAsZero)
{
	const Durin::Asset::FAssetSchemaId Alpha{
		0x71000001, 0x10000001, 0x20000001, 0x30000001};
	const Durin::Asset::FAssetSchemaId Beta{
		0x71000002, 0x10000002, 0x20000002, 0x30000002};
	const Durin::Asset::FAssetSchemaId Missing{
		0x71000003, 0x10000003, 0x20000003, 0x30000003};
	Durin::Asset::FAssetCustomVersionContainer Versions;
	EXPECT_EQ(Versions.GetVersion(Missing), 0u);
	ASSERT_TRUE(Versions.SetVersion(Beta, 7));
	ASSERT_TRUE(Versions.SetVersion(Alpha, 2));
	ASSERT_EQ(Versions.GetVersions().size(), 2u);
	EXPECT_EQ(Versions.GetVersions()[0], (Durin::Asset::FAssetCustomVersion{
		.SchemaId = Alpha, .Version = 2}));
	EXPECT_EQ(Versions.GetVersions()[1], (Durin::Asset::FAssetCustomVersion{
		.SchemaId = Beta, .Version = 7}));
	ASSERT_TRUE(Versions.SetVersion(Alpha, 0));
	EXPECT_EQ(Versions.GetVersion(Alpha), 0u);
	ASSERT_EQ(Versions.GetVersions().size(), 1u);
	EXPECT_EQ(
		Versions.SetVersion({}, 1).Error,
		Durin::Asset::EAssetError::InvalidObjectGraph);
}

TEST(FPackageAssetTests, SchemaRegistryRejectsIdentityAndDeclarationConflicts)
{
	InitializeAssetTests();
	Durin::DClass* Class = MakeSchemaTestClass("Tests::DSchemaConflictAsset");
	const Durin::Asset::FAssetSchemaId SchemaId{
		0x72000001, 0x10000001, 0x20000001, 0x30000001};
	const Durin::Asset::FAssetSchemaId OtherId{
		0x72000002, 0x10000002, 0x20000002, 0x30000002};
	ASSERT_TRUE(Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
		SchemaId,
		"Tests.Schema.Conflict",
		1,
		0,
		{Class},
		{MakeSchemaMigrationStep(0, "Tests.Schema.Conflict.0To1")})));

	EXPECT_EQ(
		Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
			SchemaId,
			"Tests.Schema.DuplicateId",
			1,
			0,
			{Class},
			{MakeSchemaMigrationStep(0, "Tests.Schema.DuplicateId.0To1")})).Error,
	...
	EXPECT_EQ(
		Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
			...
			0,
			{Class},
			{MakeSchemaMigrationStep(0, "Tests.Schema.DuplicateName.0To1")})).Error,
		Durin::Asset::EAssetError::AlreadyExists);

	const Durin::Asset::FAssetSchemaId InvalidClassId{
		0x72000003, 0x10000003, 0x20000003, 0x30000003};
	EXPECT_EQ(
		Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
			InvalidClassId,
			"Tests.Schema.DuplicateClass",
			1,
			0,
			{Class, Class},
			{MakeSchemaMigrationStep(0, "Tests.Schema.DuplicateClass.0To1")})).Error,
		Durin::Asset::EAssetError::InvalidObjectGraph);

	const Durin::Asset::FAssetSchemaId InvalidStepId{
		0x72000004, 0x10000004, 0x20000004, 0x30000004};
	EXPECT_EQ(
		Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
			InvalidStepId,
			"Tests.Schema.DuplicateStep",
			2,
			0,
			{Class},
			{
				MakeSchemaMigrationStep(0, "Tests.Schema.DuplicateStep.First"),
				MakeSchemaMigrationStep(0, "Tests.Schema.DuplicateStep.Second")
			})).Error,
		Durin::Asset::EAssetError::InvalidObjectGraph);
}

TEST(FPackageAssetTests, SchemaPlannerUsesOneOrderedChainForInspectionAndApply)
{
	InitializeAssetTests();
	Durin::DClass* Class = MakeSchemaTestClass("Tests::DSchemaChainAsset");
	const Durin::Asset::FAssetSchemaId SchemaId{
		0x73000001, 0x10000001, 0x20000001, 0x30000001};
	auto MakeReportingStep = [](Durin::uint32 FromVersion,
		Durin::Asset::EAssetCompatibilityRisk Risk)
		-> Durin::Asset::FAssetSchemaMigrationStep
	{
		const std::string HandlerId = std::format(
			"Tests.Schema.Chain.{}To{}", FromVersion, FromVersion + 1);
		const std::string Summary = std::format(
			"Migrate test schema {} -> {}.", FromVersion, FromVersion + 1);
		auto Report = [HandlerId, Summary, Risk](
			std::span<const Durin::Asset::FAssetLegacyField> Fields,
			std::vector<Durin::Asset::FAssetCompatibilityIssue>& OutIssues) {
			OutIssues.push_back({
				.LegacyFields = {Fields.begin(), Fields.end()},
				.Classification = Durin::Asset::EAssetCompatibilityClassification::Migrated,
				.MigrationSummary = Summary,
				.MigratedDataCount = Fields.size(),
				.Risk = Risk,
				.HandlerId = HandlerId});
		};
		return {
			...
			.Summary = Summary,
			.Risk = Risk,
			.ConsumedLegacyFields = {Durin::Asset::FAssetSchemaLegacyFieldId{
				.DeclaringClass = "Tests::DSchemaChainAsset",
				.Name = "Legacy",
				.TypeSignature = "int32"
			}},
			.Inspect = [Report](const Durin::Asset::FAssetPackageInspection&,
				const Durin::Asset::FAssetPackageObjectInspection&,
				std::span<const Durin::Asset::FAssetLegacyField> Fields,
				std::vector<Durin::Asset::FAssetCompatibilityIssue>& OutIssues)
				-> Durin::Asset::FAssetResult
			{
				Report(Fields, OutIssues);
				return {};
			},
			.Apply = [Report](Durin::DObject*,
				std::span<const Durin::Asset::FAssetLegacyField> Fields,
				const Durin::Asset::FAssetMigrationContext&,
				std::vector<Durin::Asset::FAssetCompatibilityIssue>& OutIssues)
				-> Durin::Asset::FAssetResult
	...
	ASSERT_TRUE(Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
		...
		{Class},
		{
			MakeReportingStep(2, Durin::Asset::EAssetCompatibilityRisk::None),
			MakeReportingStep(0, Durin::Asset::EAssetCompatibilityRisk::None),
			MakeReportingStep(
				1, Durin::Asset::EAssetCompatibilityRisk::PotentialDataLoss)
		})));

	Durin::Asset::FAssetSchemaMigrationPlan Plan;
	ASSERT_TRUE(Durin::Asset::PlanAssetSchemaMigration(SchemaId, 0, Plan));
	EXPECT_EQ(
		Plan.Status,
		Durin::Asset::EAssetSchemaMigrationStatus::MigrationAvailable);
	EXPECT_EQ(Plan.StoredVersion, 0u);
	EXPECT_EQ(Plan.CurrentVersion, 3u);
	EXPECT_EQ(
		Plan.Risk,
		Durin::Asset::EAssetCompatibilityRisk::PotentialDataLoss);
	ASSERT_EQ(Plan.Steps.size(), 3u);
	for (Durin::uint32 Index = 0; Index < 3; ++Index)
	{
		EXPECT_EQ(Plan.Steps[Index].FromVersion, Index);
		EXPECT_EQ(Plan.Steps[Index].ToVersion, Index + 1);
	}

	const std::array<Durin::Asset::FAssetLegacyField, 1> Fields = {
		Durin::Asset::FAssetLegacyField{
			.DeclaringClass = "Tests::DSchemaChainAsset",
			.Name = "Legacy",
			.TypeSignature = "int32"
		}};
	Durin::Asset::FAssetPackageInspection Inspection;
	Durin::Asset::FAssetPackageObjectInspection Object;
	Durin::Asset::FAssetMigrationContext MigrationContext;
	std::vector<Durin::Asset::FAssetCompatibilityIssue> InspectedIssues;
	std::vector<Durin::Asset::FAssetCompatibilityIssue> AppliedIssues;
	for (const Durin::Asset::FAssetSchemaMigrationStepInfo& StepInfo : Plan.Steps)
	{
		Durin::Asset::FAssetSchemaMigrationStep Step;
		ASSERT_TRUE(Durin::Asset::FindAssetSchemaMigrationStep(
			SchemaId, StepInfo.FromVersion, Step));
		EXPECT_EQ(Step.HandlerId, StepInfo.HandlerId);
		EXPECT_EQ(Step.Risk, StepInfo.Risk);
		EXPECT_EQ(Step.ConsumedLegacyFields, StepInfo.ConsumedLegacyFields);
		ASSERT_EQ(StepInfo.ConsumedLegacyFields.size(), 1u);
		EXPECT_EQ(StepInfo.ConsumedLegacyFields.front().Name, "Legacy");
		ASSERT_TRUE(Step.Inspect(Inspection, Object, Fields, InspectedIssues));
		ASSERT_TRUE(Step.Apply(nullptr, Fields, MigrationContext, AppliedIssues));
	}
	ASSERT_EQ(InspectedIssues.size(), AppliedIssues.size());
	for (size_t Index = 0; Index < InspectedIssues.size(); ++Index)
	{
		EXPECT_EQ(InspectedIssues[Index].HandlerId, AppliedIssues[Index].HandlerId);
		EXPECT_EQ(InspectedIssues[Index].Risk, AppliedIssues[Index].Risk);
		EXPECT_EQ(
			InspectedIssues[Index].MigrationSummary,
			AppliedIssues[Index].MigrationSummary);
		ASSERT_EQ(InspectedIssues[Index].LegacyFields.size(), 1u);
		ASSERT_EQ(AppliedIssues[Index].LegacyFields.size(), 1u);
		EXPECT_EQ(
			InspectedIssues[Index].LegacyFields.front().Name,
			AppliedIssues[Index].LegacyFields.front().Name);
	}
	ASSERT_TRUE(Durin::Asset::PlanAssetSchemaMigration(SchemaId, 3, Plan));
	EXPECT_EQ(Plan.Status, Durin::Asset::EAssetSchemaMigrationStatus::Current);
	EXPECT_TRUE(Plan.Steps.empty());
}

TEST(FPackageAssetTests, SchemaPlannerReportsUnknownNewerTooOldAndMissingStep)
{
	InitializeAssetTests();
	Durin::DClass* Class = MakeSchemaTestClass("Tests::DSchemaFailureAsset");
	const Durin::Asset::FAssetSchemaId GapId{
		0x74000001, 0x10000001, 0x20000001, 0x30000001};
	ASSERT_TRUE(Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
		GapId,
		"Tests.Schema.Gap",
		3,
		...
		{Class},
		{
			MakeSchemaMigrationStep(0, "Tests.Schema.Gap.0To1"),
			MakeSchemaMigrationStep(2, "Tests.Schema.Gap.2To3")
		})));
	Durin::Asset::FAssetSchemaMigrationPlan Plan;
	EXPECT_EQ(
		Durin::Asset::PlanAssetSchemaMigration(GapId, 0, Plan).Error,
		Durin::Asset::EAssetError::UnsupportedProperty);
	EXPECT_EQ(Plan.Status, Durin::Asset::EAssetSchemaMigrationStatus::MissingStep);
	ASSERT_EQ(Plan.Steps.size(), 1u);
	EXPECT_EQ(Plan.Steps.front().FromVersion, 0u);

	EXPECT_EQ(
		Durin::Asset::PlanAssetSchemaMigration(GapId, 4, Plan).Error,
		Durin::Asset::EAssetError::UnsupportedProperty);
	EXPECT_EQ(Plan.Status, Durin::Asset::EAssetSchemaMigrationStatus::NewerVersion);

	const Durin::Asset::FAssetSchemaId MinimumId{
		0x74000002, 0x10000002, 0x20000002, 0x30000002};
	ASSERT_TRUE(Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
		MinimumId,
		"Tests.Schema.Minimum",
		3,
		1,
		{Class},
		{
			MakeSchemaMigrationStep(1, "Tests.Schema.Minimum.1To2"),
			MakeSchemaMigrationStep(2, "Tests.Schema.Minimum.2To3")
		})));
	EXPECT_EQ(
		Durin::Asset::PlanAssetSchemaMigration(MinimumId, 0, Plan).Error,
		Durin::Asset::EAssetError::UnsupportedProperty);
	EXPECT_EQ(Plan.Status, Durin::Asset::EAssetSchemaMigrationStatus::TooOld);

	const Durin::Asset::FAssetSchemaId UnknownId{
		0x74000003, 0x10000003, 0x20000003, 0x30000003};
	EXPECT_EQ(
		Durin::Asset::PlanAssetSchemaMigration(UnknownId, 2, Plan).Error,
		Durin::Asset::EAssetError::UnsupportedProperty);
	EXPECT_EQ(Plan.Status, Durin::Asset::EAssetSchemaMigrationStatus::UnknownSchema);
	EXPECT_EQ(Plan.SchemaId, UnknownId);
	EXPECT_EQ(Plan.StoredVersion, 2u);
}

TEST(FPackageAssetTests, SchemaCollectionComposesDomainsAcrossClassHierarchy)
{
	InitializeAssetTests();
	Durin::DClass* BaseClass = MakeSchemaTestClass("Tests::DSchemaBaseAsset");
	Durin::DClass* DerivedClass =
		MakeSchemaTestClass("Tests::DSchemaDerivedAsset", BaseClass);
	const Durin::Asset::FAssetSchemaId AlphaId{
		0x75000001, 0x10000001, 0x20000001, 0x30000001};
	const Durin::Asset::FAssetSchemaId BetaId{
		0x75000002, 0x10000002, 0x20000002, 0x30000002};
	ASSERT_TRUE(Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
		BetaId,
		"Tests.Schema.HierarchyBeta",
		1,
		0,
		{DerivedClass},
		{MakeSchemaMigrationStep(0, "Tests.Schema.HierarchyBeta.0To1")})));
	ASSERT_TRUE(Durin::Asset::RegisterAssetSchema(MakeSchemaDescriptor(
		AlphaId,
		"Tests.Schema.HierarchyAlpha",
		1,
		0,
		{BaseClass, DerivedClass},
		{MakeSchemaMigrationStep(0, "Tests.Schema.HierarchyAlpha.0To1")})));

	std::vector<Durin::Asset::FAssetSchemaDescriptor> Schemas;
	ASSERT_TRUE(Durin::Asset::CollectAssetSchemasForClass(DerivedClass, Schemas));
	ASSERT_EQ(Schemas.size(), 2u);
	EXPECT_EQ(Schemas[0].Id, AlphaId);
	EXPECT_EQ(Schemas[1].Id, BetaId);

	Durin::Asset::FAssetCustomVersionContainer StoredVersions;
	std::vector<Durin::Asset::FAssetSchemaMigrationPlan> Plans;
	ASSERT_TRUE(Durin::Asset::PlanAssetSchemaMigrations(
		DerivedClass, StoredVersions, Plans));
	ASSERT_EQ(Plans.size(), 2u);
	EXPECT_EQ(Plans[0].SchemaId, AlphaId);
	EXPECT_EQ(Plans[1].SchemaId, BetaId);
	for (const Durin::Asset::FAssetSchemaMigrationPlan& Plan : Plans)
	{
		EXPECT_EQ(Plan.StoredVersion, 0u);
		EXPECT_EQ(Plan.CurrentVersion, 1u);
		EXPECT_EQ(
			Plan.Status,
			Durin::Asset::EAssetSchemaMigrationStatus::MigrationAvailable);
		ASSERT_EQ(Plan.Steps.size(), 1u);
		EXPECT_EQ(Plan.Steps.front().FromVersion, 0u);
		EXPECT_EQ(Plan.Steps.front().ToVersion, 1u);
	}
}
