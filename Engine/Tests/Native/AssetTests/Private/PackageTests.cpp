...
namespace
{
	...
	class DPropertyMoveBaseForTest : public Durin::DObject
	{
	public:
		explicit DPropertyMoveBaseForTest(
			const Durin::FObjectInitializer& Initializer = Durin::FObjectInitializer::Get())
			: DObject(Initializer), InlinePayload({0x11001100, 0x22002200, 0x33003300, 0x44004400}),
			  ExternalPayload({0x55005500, 0x66006600, 0x77007700, 0x88008800})
		{
		}

		static void __DefaultConstructor(const Durin::FObjectInitializer& X)
		{
			new (X.GetObj()) DPropertyMoveBaseForTest(X);
		}
		...
		static auto StaticClassNoRegister() -> Durin::DClass*
		{
			...
			{
				Class = new Durin::DClass(Durin::EC_StaticConstructor,
					"DPropertyMoveBaseForTest", sizeof(DPropertyMoveBaseForTest),
					alignof(DPropertyMoveBaseForTest), Durin::EObjectFlags::NoFlags,
					Durin::EClassFlags::None, Durin::EClassCastFlags::DClass,
					(Durin::DClass::ClassConstructorType)
						Durin::InternalConstructor<DPropertyMoveBaseForTest>);
				Class->SetSuperStructBase(Durin::DObject::StaticClass());
				Class->Register(Durin::DClass::StaticClass, "", "DPropertyMoveBaseForTest");
			}
			return Class;
		}
		...
		static auto StaticClass() -> Durin::DClass*
		{
			using namespace Durin;
			using namespace Durin::DurinCodeGen;
			static const FInt32PropertyParams ValueProp{
				"Value", EPropertyFlags::None, 1,
				STRUCT_OFFSET_UINT16(DPropertyMoveBaseForTest, Value)};
			static const FBulkDataPropertyParams InlinePayloadProp =
				FBulkDataPropertyParams::Create<FEditorBulkData>(
					"InlinePayload", EPropertyFlags::None, 1,
					STRUCT_OFFSET_UINT16(DPropertyMoveBaseForTest, InlinePayload));
			static const FBulkDataPropertyParams ExternalPayloadProp =
				FBulkDataPropertyParams::Create<FEditorBulkData>(
					"ExternalPayload", EPropertyFlags::None, 1,
					STRUCT_OFFSET_UINT16(DPropertyMoveBaseForTest, ExternalPayload));
			static const FObjectPropertyParams ReferenceProp =
				FObjectPropertyParams::ObjectPtr<DObject>(
					"ExternalReference", EPropertyFlags::None, 1,
					STRUCT_OFFSET_UINT16(DPropertyMoveBaseForTest, ExternalReference),
					&DObject::StaticClass);
			static const FPropertyParamsBase* Properties[] = {
				&ValueProp, &InlinePayloadProp, &ExternalPayloadProp, &ReferenceProp};
			static const FClassParams Params{
				&StaticClassNoRegister, "Tests::DPropertyMoveBaseForTest",
				"DPropertyMoveBaseForTest", Properties, std::size(Properties)};
			static DClass* Class = ConstructDClass(Params);
			return Class;
		}

		int32 Value = 0;
		Durin::FEditorBulkData InlinePayload;
		Durin::FEditorBulkData ExternalPayload;
		Durin::TObjectPtr<Durin::DObject> ExternalReference;
	};

	class DPropertyMoveDerivedForTest : public DPropertyMoveBaseForTest
	{
	public:
		explicit DPropertyMoveDerivedForTest(
			const Durin::FObjectInitializer& Initializer = Durin::FObjectInitializer::Get())
			: DPropertyMoveBaseForTest(Initializer)
		{
		}

		static void __DefaultConstructor(const Durin::FObjectInitializer& X)
		{
			new (X.GetObj()) DPropertyMoveDerivedForTest(X);
		}
		...
		static auto StaticClassNoRegister() -> Durin::DClass*
		{
			...
			{
				Class = new Durin::DClass(Durin::EC_StaticConstructor,
					"DPropertyMoveDerivedForTest", sizeof(DPropertyMoveDerivedForTest),
					alignof(DPropertyMoveDerivedForTest), Durin::EObjectFlags::NoFlags,
					Durin::EClassFlags::None, Durin::EClassCastFlags::DClass,
					(Durin::DClass::ClassConstructorType)
						Durin::InternalConstructor<DPropertyMoveDerivedForTest>);
				Class->SetSuperStructBase(DPropertyMoveBaseForTest::StaticClass());
				Class->Register(Durin::DClass::StaticClass, "", "DPropertyMoveDerivedForTest");
			}
			return Class;
		}
		...
		{
			static const Durin::DurinCodeGen::FClassParams Params{
				&StaticClassNoRegister, "Tests::DPropertyMoveDerivedForTest",
				"DPropertyMoveDerivedForTest", nullptr, 0};
			static Durin::DClass* Class = Durin::DurinCodeGen::ConstructDClass(Params);
			return Class;
		}
	};

	auto RegisterPropertyMoveTestRoutes(std::string& OutError) -> bool
	{
		(void)DPropertyMoveBaseForTest::StaticClass();
		(void)DPropertyMoveDerivedForTest::StaticClass();
		static const std::array Routes{
			Durin::FSerializedPropertyMove{"Tests::DPropertyMoveDerivedForTest", "Value",
				"Tests::DPropertyMoveBaseForTest", "Value"},
			Durin::FSerializedPropertyMove{"Tests::DPropertyMoveDerivedForTest", "InlinePayload",
				"Tests::DPropertyMoveBaseForTest", "InlinePayload"},
			Durin::FSerializedPropertyMove{"Tests::DPropertyMoveDerivedForTest", "ExternalPayload",
				"Tests::DPropertyMoveBaseForTest", "ExternalPayload"},
			Durin::FSerializedPropertyMove{"Tests::DPropertyMoveDerivedForTest", "ExternalReference",
				"Tests::DPropertyMoveBaseForTest", "ExternalReference"}};
		return Durin::RegisterSerializedPropertyMoves(Routes, &OutError);
	}
	...
	auto MoveSchemaTestFieldOwner(Durin::ObjectPackage::FLinkerTables& Linker,
		std::string_view CurrentOwner, std::string_view StoredOwner,
		std::span<const std::string_view> Names) -> void
	{
		using namespace Durin::ObjectPackage;
		auto CurrentSchema = std::ranges::find(
			Linker.Schemas, CurrentOwner, &FSerializedSchema::QualifiedName);
		ASSERT_NE(CurrentSchema, Linker.Schemas.end());
		auto StoredSchema = std::ranges::find(
			Linker.Schemas, StoredOwner, &FSerializedSchema::QualifiedName);
		if (StoredSchema == Linker.Schemas.end())
		{
			Linker.Schemas.push_back({std::string(StoredOwner), {}});
			StoredSchema = std::prev(Linker.Schemas.end());
			CurrentSchema = std::ranges::find(
				Linker.Schemas, CurrentOwner, &FSerializedSchema::QualifiedName);
		}
		for (std::string_view Name : Names)
		{
			auto Field = std::ranges::find(
				CurrentSchema->Fields, Name, &FSerializedField::Name);
			ASSERT_NE(Field, CurrentSchema->Fields.end());
			StoredSchema->Fields.push_back(*Field);
			CurrentSchema->Fields.erase(Field);
			for (auto& Export : Linker.Exports)
				for (auto& Property : Export.Properties)
					if (Property.DeclaringType == CurrentOwner && Property.FieldName == Name)
						Property.DeclaringType = StoredOwner;
		}
	}
} // namespace

TEST(FPackageAssetTests, CrossOwnerPropertyRoutesPreserveBulkReferencesAndAuthoredIntent)
{
	InitializeAssetTests();
	using namespace Durin;
	std::string Error;
	ASSERT_TRUE(RegisterPropertyMoveTestRoutes(Error)) << Error;
	FPackagePath Path, TargetPath;
	ASSERT_TRUE(FPackagePath::TryCreate("/TestAssets/PropertyMove", Path));
	ASSERT_TRUE(FPackagePath::TryCreate("/TestAssets/PropertyMoveDependency", TargetPath));
	...
	DPropertyMoveDerivedForTest* Source = nullptr;
	ASSERT_TRUE(CreatePackageLeafAssetForTesting(Path, Source));
	Source->Value = 0;
	const std::array InlineBytes{std::byte{0x21}, std::byte{0x43}, std::byte{0x65}};
	FByteArray ExternalBytes(static_cast<size_t>(EditorBulkDataExternalThreshold + 11),
		std::byte{0x7a});
	ASSERT_TRUE(Source->InlinePayload.ReplaceBytes(InlineBytes));
	ASSERT_TRUE(Source->ExternalPayload.ReplaceBytes(ExternalBytes));
	...
	const FAuthoredOverridePath ForcedPath{FAuthoredOverridePathToken::Field(
		FName("Tests::DPropertyMoveBaseForTest"), FName("Value"))};
	ASSERT_TRUE(Source->SetAuthoredOverride(
		ForcedPath, EAuthoredOverrideProvenance::Forced, &OverrideDiagnostic));
	ASSERT_TRUE(SavePackage(Source->GetPackage()));
	ASSERT_NO_FATAL_FAILURE(RewriteSchemaTestPackage(Path, [](auto& Linker) {
		constexpr std::array<std::string_view, 4> Names{
			"Value", "InlinePayload", "ExternalPayload", "ExternalReference"};
		MoveSchemaTestFieldOwner(Linker, "Tests::DPropertyMoveBaseForTest",
			"Tests::DPropertyMoveDerivedForTest", Names);
	}));
	const FAssetCatalogEntry SourceData = FindAssetExact(Path);
	ASSERT_TRUE(SourceData);
	FByteArray Before, After;
	ASSERT_TRUE(FFileHelper::LoadFileToArray(Before, SourceData->PhysicalPath));
	FAssetPackageInspection PackageInspection;
	ASSERT_TRUE(InspectAssetPackage(SourceData->PhysicalPath, PackageInspection));
	...
	ASSERT_TRUE(AssetPrivate::DastV9::GetCodec().InspectSchema(ByteSource, Path,
		FReflectionSchemaCatalog::Capture(), SchemaInspection, nullptr, false, {}));
	EXPECT_EQ(SchemaInspection.Status, EPackageSchemaStatus::Compatible);
	EXPECT_EQ(SchemaInspection.CanonicalizationEvidence.size(), 4u);

	ASSERT_TRUE(UnloadPackage(Path));
	ASSERT_TRUE(UnloadPackage(TargetPath));
	DPackage* LoadedPackage = nullptr;
	FAssetLoadReport Report;
	const FAssetResult LoadResult = LoadPackage(Path, LoadedPackage, &Report);
	ASSERT_TRUE(LoadResult) << LoadResult.Message;
	auto* Loaded = Cast<DPropertyMoveDerivedForTest>(
		LoadedPackage->FindTopLevelAsset(Path.GetPackageName()));
	ASSERT_NE(Loaded, nullptr);
	EXPECT_EQ(Loaded->ExternalReference, FindResidentPackage(TargetPath)
		->FindTopLevelAsset(TargetPath.GetPackageName()));
	EXPECT_TRUE(std::ranges::equal(
		Loaded->InlinePayload.GetPayload().Wait().Buffer.GetBytes(), InlineBytes));
	EXPECT_TRUE(std::ranges::equal(
		Loaded->ExternalPayload.GetPayload().Wait().Buffer.GetBytes(), ExternalBytes));
	const auto Overrides = Loaded->GetAuthoredOverrideEntries();
	const auto Forced = std::ranges::find_if(Overrides, [&](const auto& Entry) {
		return CompareAuthoredOverridePaths(Entry.Path, ForcedPath) == 0;
	});
	ASSERT_NE(Forced, Overrides.end());
	EXPECT_EQ(Forced->Provenance, EAuthoredOverrideProvenance::Forced);
	EXPECT_FALSE(LoadedPackage->IsDirty());
	EXPECT_TRUE(LoadedPackage->IsCanonicalResaveRecommended());
	ASSERT_TRUE(FFileHelper::LoadFileToArray(After, SourceData->PhysicalPath));
	EXPECT_EQ(Before, After);
	ASSERT_TRUE(SavePackage(LoadedPackage));
	ASSERT_TRUE(FFileHelper::LoadFileToArray(After, SourceData->PhysicalPath));
	EXPECT_NE(Before, After);
	ObjectPackage::FLinkerTables CanonicalLinker;
	ObjectPackage::FPackageReaderDiagnostic ReadDiagnostic;
	...
	ASSERT_TRUE(ObjectPackage::ReadPackageV9(
		After, Bulk, Path, CanonicalLinker, &ReadDiagnostic)) << ReadDiagnostic.Message;
	for (const auto& Property : CanonicalLinker.Exports.front().Properties)
		EXPECT_NE(Property.DeclaringType, "Tests::DPropertyMoveDerivedForTest");
	ASSERT_TRUE(UnloadPackage(Path));
	ASSERT_TRUE(UnloadPackage(TargetPath));
	ASSERT_TRUE(DeleteAssetClosureForTest({Path, TargetPath}));
}

TEST(FPackageAssetTests, CrossOwnerPropertyRoutesRejectDuplicateAndIncompatibleIdentities)
{
	using namespace Durin;
	for (const bool bDuplicate : {true, false})
	{
		InitializeAssetTests();
		std::string Error;
		ASSERT_TRUE(RegisterPropertyMoveTestRoutes(Error)) << Error;
		FPackagePath Path;
		ASSERT_TRUE(FPackagePath::TryCreate(bDuplicate
			? "/TestAssets/PropertyMoveDuplicate"
			: "/TestAssets/PropertyMoveIncompatible", Path));
		DPropertyMoveDerivedForTest* Source = nullptr;
		ASSERT_TRUE(CreatePackageLeafAssetForTesting(Path, Source));
		Source->Value = 9;
		ASSERT_TRUE(SavePackage(Source->GetPackage()));
		ASSERT_NO_FATAL_FAILURE(RewriteSchemaTestPackage(Path, [&](auto& Linker) {
			constexpr std::array<std::string_view, 1> Names{"Value"};
			if (bDuplicate)
			{
				using namespace ObjectPackage;
				auto CurrentSchema = std::ranges::find(Linker.Schemas,
					"Tests::DPropertyMoveBaseForTest", &FSerializedSchema::QualifiedName);
				ASSERT_NE(CurrentSchema, Linker.Schemas.end());
				auto CurrentField = std::ranges::find(
					CurrentSchema->Fields, "Value", &FSerializedField::Name);
				ASSERT_NE(CurrentField, CurrentSchema->Fields.end());
				auto StoredSchema = std::ranges::find(Linker.Schemas,
					"Tests::DPropertyMoveDerivedForTest", &FSerializedSchema::QualifiedName);
				if (StoredSchema == Linker.Schemas.end())
				{
					Linker.Schemas.push_back({"Tests::DPropertyMoveDerivedForTest", {*CurrentField}});
				}
				else StoredSchema->Fields.push_back(*CurrentField);
				auto CurrentProperty = std::ranges::find_if(
					Linker.Exports.front().Properties, [](const auto& Property) {
						return Property.DeclaringType == "Tests::DPropertyMoveBaseForTest"
							&& Property.FieldName == "Value";
					});
				ASSERT_NE(CurrentProperty, Linker.Exports.front().Properties.end());
				auto StoredProperty = *CurrentProperty;
				StoredProperty.DeclaringType = "Tests::DPropertyMoveDerivedForTest";
				Linker.Exports.front().Properties.push_back(std::move(StoredProperty));
			}
			else
			{
				MoveSchemaTestFieldOwner(Linker, "Tests::DPropertyMoveBaseForTest",
					"Tests::DPropertyMoveDerivedForTest", Names);
				for (auto& Schema : Linker.Schemas)
					if (Schema.QualifiedName == "Tests::DPropertyMoveDerivedForTest")
						for (auto& Field : Schema.Fields)
							if (Field.Name == "Value") Field.Type = {
								.Kind = ObjectPackage::EValueKind::String};
				for (auto& Property : Linker.Exports.front().Properties)
					if (Property.DeclaringType == "Tests::DPropertyMoveDerivedForTest"
						&& Property.FieldName == "Value")
					{
						Property.Type = {.Kind = ObjectPackage::EValueKind::String};
						Property.Value = {.Text = "invalid"};
					}
			}
		}));
		ASSERT_TRUE(UnloadPackage(Path));
		DPackage* Loaded = nullptr;
		const FAssetResult Result = LoadPackage(Path, Loaded);
		EXPECT_EQ(Result.Error, bDuplicate
			? EAssetError::CorruptFile : EAssetError::UnsupportedProperty);
