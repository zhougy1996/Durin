...
TEST(FStandaloneCookProcessTests, CooksSavedFamiliesAndReusesValidatedOutputs)
{
	...
	ASSERT_TRUE(Cube->SetSourceData(CubeInput, Error)) << Error;
	ASSERT_TRUE(Cube->SetBuildSettings(ETextureCubeSourceLayout::SixFaces, 4, 0, 4, 4, true, Error)) << Error;
	...
	ASSERT_TRUE(Volume->SetSourceData(Voxels, Error)) << Error;
