...
namespace Durin
{
	...
	TEST(FTaskAdmissionTests, PreparingCancellationPreservesOriginalFailureAttribution)
	{
		...
		FEngineThreadPoolTestGuard Guard;
		ASSERT_TRUE(InitializeTaskScheduler(1));
		auto Failed = LaunchTask("CompletedFailure", [] { throw std::runtime_error("original failure"); });
		ASSERT_EQ(ETaskState::Failed, WaitTask(Failed).TaskState);
		auto Edge = Durin::Then(Failed, "AlreadyFailedEdge", [] {});
	...
	TEST(FTaskAttributionTests, AggregatesLifecycleRejectionsResultsAndConcurrentSnapshots)
	{
		...
		auto Producer = LaunchUniqueTask<Durin::FByteBuffer>(
			"AggregateUniqueProducer",
			[]() { return Durin::FByteBuffer(128, std::byte{7}); },
			MainOptions,
			128);
		...
		FTaskHandle Consumer = ConsumeThen(std::move(Producer), "AggregateUniqueConsumer", [](Durin::FByteBuffer&& Value) {
