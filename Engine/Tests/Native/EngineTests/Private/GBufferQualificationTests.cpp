	...
	FourFamilyPassMeetsFrozenRTX3090TimingAndMemoryGates)
{
	...
	PointA.Range = 6.0f;
	Scene.AddOrReplaceLight(Durin::FLightSceneId(20),
		std::make_unique<Durin::FPointLightSceneProxy>(PointA));
	...
	Scene.AddOrReplaceLight(Durin::FLightSceneId(21),
		std::make_unique<Durin::FSpotLightSceneProxy>(SpotA));
	auto PointB = PointA;
	PointB.Position = {-0.5, 1.25, 1.5};
	PointB.Color = {0.1f, 1.0f, 0.2f};
	PointB.Intensity = 3.0f;
	PointB.Range = 4.0f;
	Scene.AddOrReplaceLight(Durin::FLightSceneId(22),
		std::make_unique<Durin::FPointLightSceneProxy>(PointB));
	auto SpotB = SpotA;
	...
	Scene.AddOrReplaceLight(Durin::FLightSceneId(23),
		std::make_unique<Durin::FSpotLightSceneProxy>(SpotB));
	Durin::FlushRenderingCommands();
	...
	UnlitPublication.LocalLayer.Parameters.push_back({
		.Id = Durin::MaterialParameters::BaseColorId,
		.Type = Durin::EMaterialParameterType::Vector,
		.VectorValue = {0.1, 0.8, 0.3}});
	...
	TranslucentPublication.LocalLayer.StaticProperties =
		Durin::FMaterialStaticProperties{
			...
			.ShadingModel = Durin::EMaterialShadingModel::Unlit};
	TranslucentPublication.LocalLayer.Parameters.push_back({
		.Id = Durin::MaterialParameters::BaseColorId,
		.Type = Durin::EMaterialParameterType::Vector,
		.VectorValue = {0.8, 0.2, 0.5}});
	TranslucentPublication.LocalLayer.Parameters.push_back({
		.Id = Durin::MaterialParameters::OpacityId,
	...
	Durin::FlushRenderingCommands();
	Scene.AddOrReplacePrimitive(Durin::FPrimitiveSceneId(5),
		std::make_unique<Durin::FStaticMeshSceneProxy>(StaticQuad.get(),
			std::vector<Durin::FMaterialRenderProxyRef>{UnlitMaterial}, 1),
		Translate(-0.55, -0.45));
	Scene.AddOrReplacePrimitive(Durin::FPrimitiveSceneId(6),
		std::make_unique<Durin::FStaticMeshSceneProxy>(StaticQuad.get(),
			std::vector<Durin::FMaterialRenderProxyRef>{TranslucentMaterial}, 1),
		Translate(-0.35, -0.25));
	Durin::FlushRenderingCommands();
	...
	auto ProfileProductionInterval = [&RenderProductionFrames,
		&WaitForProductionQueries](auto& Queries, auto& Durations,
			auto& QuerySlot,
	...
	ProfileProductionInterval(ProductionGBufferQueries,
		ProductionGBufferDurations,
		GGBufferTimingQueries, Durin::SetGBufferTimingQuerySink,
		CaptureGBufferTiming);
	ProfileProductionInterval(ProductionDeferredQueries,
		ProductionDeferredDurations,
		GDeferredTimingQueries, Durin::SetDeferredDirectionalTimingQuerySink,
		CaptureDeferredTiming);
	bEnableProductionAmbientOcclusion = false;
	ProfileProductionInterval(ProductionDeferredQueries,
		ProductionDeferredWithoutAODurations,
		GDeferredTimingQueries, Durin::SetDeferredDirectionalTimingQuerySink,
		CaptureDeferredTiming);
	bEnableProductionAmbientOcclusion = true;
	ProfileProductionInterval(ProductionSceneQueries,
		ProductionSceneDurations,
		GSceneTimingQueries, Durin::SetSceneColorTimingQuerySink,
		CaptureSceneTiming);
	ProfileProductionInterval(ProductionPostProcessQueries,
		ProductionPostProcessDurations,
		GPostProcessTimingQueries, Durin::SetPostProcessTimingQuerySink,
		CapturePostProcessTiming);
	ProfileProductionInterval(ProductionShadowQueries,
		ProductionShadowDurations,
		GShadowTimingQueries, Durin::SetShadowDepthTimingQuerySink,
		CaptureShadowTiming);
	...
	for (size_t Index = 0; Index < MeasuredFrames; ++Index)
	{
		ProductionRetainedDurations.push_back(
			ProductionSceneDurations[Index] > ProductionDeferredDurations[Index]
	...
	const Durin::uint64 ProductionAmbientOcclusionCompositionIncrement =
		ProductionDeferredMedian > ProductionDeferredWithoutAOMedian
			? ProductionDeferredMedian - ProductionDeferredWithoutAOMedian
			: 0u;
	...
	EXPECT_EQ(GLastCounters.GroundTruthAmbientOcclusionActiveBytes,
		Durin::FGroundTruthAmbientOcclusionRenderer::CalculateTargetBytes(
			TimingWidth, TimingHeight));
	EXPECT_GE(GLastCounters.GroundTruthAmbientOcclusionRetainedBytes,
		GLastCounters.GroundTruthAmbientOcclusionActiveBytes);
	EXPECT_LE(GLastCounters.GroundTruthAmbientOcclusionRetainedBytes,
		Durin::FGroundTruthAmbientOcclusionRenderer::MaximumRetainedBytes);
	...
	EXPECT_EQ(GLastCounters.GroundTruthAmbientOcclusionActiveBytes,
		2u * 384u * 216u);
	Scene.UpdatePrimitiveTransform(Durin::FPrimitiveSceneId(1),
		Translate(-0.9, -0.8));
	Scene.UpdatePrimitiveTransform(Durin::FPrimitiveSceneId(2),
		Translate(0.1, -0.9));
	Scene.UpdatePrimitiveTransform(Durin::FPrimitiveSceneId(3),
		Translate(-0.9, 0.1));
	Scene.UpdatePrimitiveTransform(Durin::FPrimitiveSceneId(4),
		Translate(0.1, 0.1));
	...
	MutatedPublication.LocalLayer.StaticProperties =
		Durin::FMaterialStaticProperties{
			...
			.ShadingModel = Durin::EMaterialShadingModel::Lit,
			.bTwoSided = true};
	MutatedPublication.LocalLayer.Parameters.push_back({
		.Id = Durin::MaterialParameters::BaseColorId,
		.Type = Durin::EMaterialParameterType::Vector,
		.VectorValue = {0.2, 0.55, 0.8}});
