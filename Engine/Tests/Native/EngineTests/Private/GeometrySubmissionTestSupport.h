...
namespace Durin::Tests
{
	...
	public:
		...
		std::array<FMeshGeometryRecord::FRef, 2> Publications;
		auto PublishGeometry() -> bool
		{
			FProceduralProxy Source;
			Source.Geometry = Geometry;
			FMeshCollectionContext Context;
			Context.PrimitiveId = FPrimitiveComponentId(1);
			Context.WorldBounds = GetLocalBounds();
			FMeshBatchCollector Collector(EMeshCollectionPurpose::Receiver);
			Source.CollectMeshBatches(Context, Collector);
			if (Collector.GetBatches().size() != Publications.size()) return false;
			std::array<FMeshGeometryRecord::FRef, 2> Candidate;
			for (size_t I = 0; I < Candidate.size(); ++I)
			{
				const auto& Element = Collector.GetBatches()[I].Elements[0];
				FMeshGeometryElement GeometryElement;
				GeometryElement.ElementId = Element.ElementId;
				GeometryElement.Draw = Element.Draw;
				GeometryElement.Vertices = Element.Vertices;
				GeometryElement.Indices = Element.Indices;
				GeometryElement.InstanceStreams = Element.InstanceStreams;
				GeometryElement.LocalBounds = Element.LocalBounds;
				auto Record = FMeshGeometryRecord::Publish(std::make_unique<FProceduralBinding>(*Geometry.Binding),
					{std::move(GeometryElement)});
				if (!Record) return false;
				Candidate[I] = std::move(*Record);
			}
			Publications = std::move(Candidate);
			return true;
		}
		...
		auto CollectMeshBatches(const FMeshCollectionContext& Context, FMeshBatchCollector& Collector) const -> void override
		{
			...
			for (uint64 I = 0; I < 2; ++I)
			{
				...
				{
					Batch.GeometryRecord = Publications[Index];
					...
					Batch.PublishedMaterials = {Materials[Index]};
	...
	public:
		...
		auto Prepare(FRHICommandListImmediate& CommandList, const FRHIUniformBufferRange& Transform,
			const FVertexFactoryBinding& Binding) const -> std::shared_ptr<const FRHIShaderParameterBatch> override
		{
			EXPECT_FALSE(CommandList.IsInsideRenderPass());
	...
	public:
		...
		inline static std::atomic<uint64> SupportQueries = 0;
		auto Supports(uint32 Pass, const FGeometryDrawRange& Draw) const -> bool override
		{
			SupportQueries.fetch_add(1, std::memory_order_relaxed);
			return FMeshVertexFactoryImplementation::Supports(Pass, Draw);
		}
