...
TEST(FStaticMeshMaterialTests, ImportedStaticMeshBuildsLODSectionsAndMaterialSlots)
{
	...
	Durin::FStaticMeshImportResult ImportResult = Durin::StandardAssetImport::ImportStaticMeshAsset(Source.generic_string(), "/MeshImportTests/MultiSection");
...
TEST(FStaticMeshMaterialTests, StaticMeshSourceProvenanceLivesOutsideContentAndSurvivesAssetOperations)
{
	...
	const std::filesystem::path Source = std::filesystem::path(DURIN_TEST_DATA_DIR) / "MultiSection.gltf";
	Durin::FStaticMeshImportResult Import = Durin::StandardAssetImport::ImportStaticMeshAsset(
	...
	EXPECT_EQ(Durin::StandardAssetImport::InspectStaticMeshSource(*Import.Asset).Status,
...
TEST(FStaticMeshMaterialTests, StaticMeshWithoutSourceMetadataLoadsAndMissingSourceCanBeRepaired)
{
	...
	ASSERT_EQ(Durin::StandardAssetImport::InspectStaticMeshSource(*Mesh).Status,
	...
	EXPECT_EQ(Durin::StandardAssetImport::InspectStaticMeshSource(*Mesh).Status,
		Durin::EStaticMeshSourceStatus::Invalid);
	*SourceImportData = {};
	std::string RepairError;
	const std::filesystem::path Source = std::filesystem::path(DURIN_TEST_DATA_DIR) / "MultiSection.gltf";
	ASSERT_TRUE(Durin::StandardAssetImport::IngestAndChangeStaticMeshSource(*Mesh,
	...
	const Durin::FStaticMeshSourceDiagnostic Missing =
		Durin::StandardAssetImport::InspectStaticMeshSource(*Mesh);
...
TEST(FStaticMeshMaterialTests, StaticMeshMaterialSlotDefinitionsRoundTripWithDefaults)
{
	...
	const std::filesystem::path Source = std::filesystem::path(DURIN_TEST_DATA_DIR) / "MultiSection.gltf";
	Durin::FStaticMeshImportResult Import = Durin::StandardAssetImport::ImportStaticMeshAsset(Source.generic_string(), MeshPath.ToString());
	ASSERT_TRUE(Import) << Import.Message;
...
TEST(FStaticMeshMaterialTests, StaticMeshMaterialSlotReconciliationPreservesStableIndices)
{
	...
	auto ImportBase = [&](std::string_view Name) -> Durin::DStaticMesh* {
		...
		Durin::FStaticMeshImportResult Import = Durin::StandardAssetImport::ImportStaticMeshAsset(BaseSource.generic_string(), AssetPath);
...
TEST(FStaticMeshMaterialTests, FixedRowAssignmentRoundTripsByIndex)
{
	...
	const std::filesystem::path BaseSource = std::filesystem::path(DURIN_TEST_DATA_DIR) / "MultiSection.gltf";
	Durin::FStaticMeshImportResult Import = Durin::StandardAssetImport::ImportStaticMeshAsset(BaseSource.generic_string(), MeshPath.ToString());
	ASSERT_TRUE(Import) << Import.Message;
...
TEST(FStaticMeshMaterialTests, StaticMeshImportSettingsPersistAcrossSourceRebuild)
{
	...
	Durin::FStaticMeshImportResult ImportResult = Durin::StandardAssetImport::ImportStaticMeshAsset(
...
TEST(FStaticMeshMaterialTests, StaticMeshComponentOverridesRoundTripAfterMeshDependenciesLoad)
{
	...
	const std::filesystem::path Source = std::filesystem::path(DURIN_TEST_DATA_DIR) / "MultiSection.gltf";
	Durin::FStaticMeshImportResult MeshImport = Durin::StandardAssetImport::ImportStaticMeshAsset(Source.generic_string(), MeshPath.ToString());
...
TEST(FStaticMeshMaterialTests, MaterialInstanceAssetsRoundTripParentAndOverrides)
{
	...
	Durin::FTexture2DImportResult TextureImport = Durin::StandardAssetImport::ImportTexture2DAsset(TextureSource.generic_string(), TexturePath.ToString());
