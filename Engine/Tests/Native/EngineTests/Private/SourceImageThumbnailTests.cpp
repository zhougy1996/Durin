...
namespace Durin::Editor::ContentBrowser::Private
{
	...
	TEST(FSourceImageThumbnailTests, DecodeExceptionsReleaseAllSlotsAndFailEntries)
	{
		ShutdownTaskScheduler(false);
		struct FTaskSchedulerShutdownGuard
		{
		...
		ASSERT_TRUE(InitializeTaskScheduler(1));
		FSourceImageThumbnailCache Cache;
		Cache.SetBeforeDecodeForTesting([] { throw std::runtime_error("Injected decode failure"); });
		Cache.BeginFrame();
		for (uint32 Index = 0; Index < 4; ++Index)
		{
			const std::string Path = "ThrowingThumbnail" + std::to_string(Index) + ".png";
			Cache.Request({.Identity = Path, .PhysicalPath = Path});
		}
		Cache.EndFrame();
		ASSERT_EQ(Cache.GetTrackedTaskCountForTesting(), 4u);
		for (uint32 Attempt = 0; Attempt < 1'000 && Cache.GetTrackedTaskCountForTesting() != 0; ++Attempt)
		{
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
			Cache.BeginFrame();
		}
		...
		for (uint32 Index = 0; Index < 4; ++Index)
		{
			const auto View = Cache.Find("ThrowingThumbnail" + std::to_string(Index) + ".png");
			EXPECT_EQ(View.State, Editor::EAssetThumbnailState::Failed);
			EXPECT_FALSE(View.Diagnostic.empty());
		}
		Cache.SetBeforeDecodeForTesting({});
		Cache.Request({.Identity = "Next", .PhysicalPath = "MissingThumbnailAfterExceptions.png"});
		Cache.EndFrame();
		EXPECT_EQ(Cache.GetTrackedTaskCountForTesting(), 1u);
		for (uint32 Attempt = 0; Attempt < 1'000 && Cache.GetTrackedTaskCountForTesting() != 0; ++Attempt)
		{
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
			Cache.BeginFrame();
		}
		EXPECT_EQ(Cache.Find("Next").State, Editor::EAssetThumbnailState::Failed);
		EXPECT_EQ(Cache.GetTrackedTaskCountForTesting(), 0u);
	}

	TEST(FSourceImageThumbnailTests, ScopeCancellationFinalizesDecodeWithoutResult)
	{
		ShutdownTaskScheduler(false);
		struct FTaskSchedulerShutdownGuard
		{
		...
		FSourceImageThumbnailCache Cache(Scope.GetToken());
		auto Release = std::make_shared<std::atomic<bool>>(false);
		Cache.SetBeforeDecodeForTesting([Release] {
			while (!Release->load()) std::this_thread::yield();
		});
		Cache.BeginFrame();
		for (uint32 Index = 0; Index < 4; ++Index)
		{
			const std::string Path = "CanceledThumbnail" + std::to_string(Index) + ".png";
			Cache.Request({.Identity = Path, .PhysicalPath = Path});
		}
		Cache.EndFrame();
		...
		EXPECT_EQ(Cache.GetTrackedTaskCountForTesting(), 0u);
		for (uint32 Index = 0; Index < 4; ++Index)
			EXPECT_EQ(Cache.Find("CanceledThumbnail" + std::to_string(Index) + ".png").State,
				Editor::EAssetThumbnailState::Failed);
	}
