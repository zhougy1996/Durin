...
namespace
{
	...
	template <typename T>
	auto AppendLittleEndian(Durin::FByteBuffer& Bytes, T Value) -> void
	{
		using U = std::make_unsigned_t<T>;
		const U Bits = static_cast<U>(Value);
		for (size_t Byte = 0; Byte < sizeof(T); ++Byte)
			Bytes.push_back(static_cast<std::byte>(Bits >> (Byte * 8)));
	}

	template <typename T>
	auto WriteLittleEndian(Durin::FByteBuffer& Bytes, size_t Offset, T Value) -> void
	{
		using U = std::make_unsigned_t<T>;
		const U Bits = static_cast<U>(Value);
		for (size_t Byte = 0; Byte < sizeof(T); ++Byte)
			Bytes[Offset + Byte] = static_cast<std::byte>(Bits >> (Byte * 8));
	}

	template <typename T>
	auto ReadLittleEndian(FByteView Bytes, size_t Offset) -> T
	{
		using U = std::make_unsigned_t<T>;
		U Bits = 0;
		for (size_t Byte = 0; Byte < sizeof(T); ++Byte)
	...
	auto AppendString(Durin::FByteBuffer& Bytes, std::string_view Value) -> void
	{
		AppendLittleEndian<uint64>(Bytes, Value.size());
		const FByteView ValueBytes =
	...
	auto BuildCollisionKeyBytes(
		...
		uint32 TargetPlatform) -> Durin::FByteBuffer
	{
	...
	auto BuildPrototypePayload(const FCollisionSourceFixture& Fixture) -> Durin::FByteBuffer
	{
		...
		Durin::FByteBuffer Bytes(CollisionPayloadHeaderSize
		...
		for (size_t ChunkIndex = 0; ChunkIndex < Chunks.size(); ++ChunkIndex)
		{
			...
			Bytes.resize(Aligned, std::byte{0});
		...
		WriteLittleEndian<uint64>(Bytes, 48,
			FXxHash64::HashBuffer(FByteView(Bytes).subspan(64)).HashValue);
		return Bytes;
	}

	auto ValidatePrototypePayload(FByteView Bytes) -> bool
	{
...
DURIN_STATIC_MESH_COLLISION_ROUTINE_TEST(FPhysicsCookedCollisionStage0Tests, FreezesKeyFormatPayloadAndDescriptorIdentity)
{
	...
	const Durin::FByteBuffer First = BuildKey(Source, Geometry,
	...
	const Durin::FByteBuffer Second = BuildKey(Source, Geometry,
		ECollisionSourceMode::TriangleMeshFromLOD0, ECollisionQueryPolicy::SimpleAndComplex,
		std::bit_cast<uint32>(1.0e-5f), "Assimp", 602, {5, 0, 2}, 1);
	...
	const Durin::FByteBuffer Payload = BuildPrototypePayload(MakeTetrahedron());
	...
	auto ExpectCorrupt = [&](size_t Offset) {
		Durin::FByteBuffer Corrupt = Payload;
...
DURIN_STATIC_MESH_COLLISION_ROUTINE_TEST(FPhysicsCookedCollisionStage3Tests, StaticMeshAuthorshipInvalidatesFailedCollisionAndRetries)
{
	...
	ASSERT_NE(Mesh, nullptr);
	std::string Error;
	FStaticMeshCollisionBuildResult Cold;
	ASSERT_TRUE(BuildStaticMeshCollisionDerivedData(*Mesh->GetRenderData(),
		EBodySetupCollisionSourceMode::TriangleMeshFromLOD0,
		EBodySetupCollisionQueryPolicy::SimpleAndComplex, Cold)) << Error;
	...
	FStaticMeshCollisionBuildResult Warm;
	ASSERT_TRUE(BuildStaticMeshCollisionDerivedData(*Mesh->GetRenderData(),
		EBodySetupCollisionSourceMode::TriangleMeshFromLOD0,
		EBodySetupCollisionQueryPolicy::SimpleAndComplex, Warm)) << Error;
	EXPECT_EQ(Warm.Origin, EStaticMeshBuildOrigin::CacheHit);
	EXPECT_EQ(Warm.DerivedDataKey, FirstKey);
	...
	ASSERT_TRUE(Rejected.DerivedDataCause->RecipeCause);
	...
	EXPECT_EQ(Rejected.Mode, EBodySetupCollisionSourceMode::ConvexHullFromLOD0);
	EXPECT_EQ(Rejected.DerivedDataCause->Code, EStaticMeshDerivedDataError::Recipe);
	FStaticMeshCollisionBuildResult Changed;
	ASSERT_TRUE(BuildStaticMeshCollisionDerivedData(*Mesh->GetRenderData(),
		EBodySetupCollisionSourceMode::TriangleMeshFromLOD0,
		EBodySetupCollisionQueryPolicy::ComplexOnly, Changed)) << Error;
