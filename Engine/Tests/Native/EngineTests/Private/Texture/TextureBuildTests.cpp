...
namespace
{
	...
	public:
		...
		auto Build(
			...
			const Durin::FTexture2DRecipeExecutionControl*) -> std::expected<Durin::FTexture2DRecipeBuildProduct, Durin::FTexture2DBuildError> override
		{
			return Durin::FTexture2DRecipeBuildProduct{};
		}
	};
}
...
TEST(FTextureSourceTests, InitApisKeepCanonicalIdentityAcrossLosslessStorage)
{
	...
	auto ImageResult1 = Durin::Image::FImage::TryCreate({.Width = 4, .Height = 4,
		.Format = Durin::Image::ERawImageFormat::RGBA8,
		.GammaSpace = Durin::Image::EImageGammaSpace::SRGB}, Pixels);
	...
	auto ImageResult2 = Durin::Image::FImage::TryCreate(Image.GetInfo(), UpdatedPixels);
	ASSERT_TRUE(ImageResult2);
	auto UpdatedImage = std::move(*ImageResult2);
...
TEST(FTextureSourceTests, ZstdFallsBackAndRejectsDamagedFrames)
{
	using namespace Durin;
	Image::FImage Image;
	auto ImageResult3 = Image::FImage::TryCreate({.Width = 1, .Height = 1,
		.Format = Image::ERawImageFormat::RGBA8}, FByteBuffer(4, std::byte{9}));
	...
	auto ImageResult4 = Image::FImage::TryCreate({.Width = 64, .Height = 64,
		.Format = Image::ERawImageFormat::RGBA8}, FByteBuffer(64 * 64 * 4, std::byte{9}));
	ASSERT_TRUE(ImageResult4);
	Image = std::move(*ImageResult4);
	ASSERT_TRUE(Source.Init2D(Image.GetView(), 4));
...
TEST(FTextureSourceTests, RejectsUnknownCodecAndCanonicalHashMismatch)
{
	...
	Image::FImage Image;
	auto ImageResult5 = Image::FImage::TryCreate({.Width = 16, .Height = 16,
		.Format = Image::ERawImageFormat::RGBA8}, FByteBuffer(1024, std::byte{31}));
	ASSERT_TRUE(ImageResult5);
	Image = std::move(*ImageResult5);
	FTextureSource Source;
	ASSERT_TRUE(Source.Init2D(Image.GetView(), 4));
...
TEST(FTextureSourceTests, StorageOnlyCommitKeepsCookedPixelsAndOwner)
{
	...
	Image::FImage Image;
	auto ImageResult6 = Image::FImage::TryCreate({.Width = 16, .Height = 16,
		.Format = Image::ERawImageFormat::RGBA8, .GammaSpace = Image::EImageGammaSpace::SRGB}, FByteBuffer(16 * 16 * 4, std::byte{71}));
	ASSERT_TRUE(ImageResult6);
	Image = std::move(*ImageResult6);
	FTextureSource Source;
...
TEST(FTextureSourceTests, Texture2DPreservesSuppliedMipChainForRecipeBuild)
{
	...
	for (uint32 Index = 0; Index < 3; ++Index)
	{
		...
		auto ImageResult7 = Durin::Image::FImage::TryCreate({.Width = Size, .Height = Size,
			.Format = Durin::Image::ERawImageFormat::RGBA8,
			.GammaSpace = Durin::Image::EImageGammaSpace::SRGB}, Durin::FByteBuffer(Size * Size * 4, static_cast<std::byte>(Index * 70)));
		ASSERT_TRUE(ImageResult7);
		Images[Index] = std::move(*ImageResult7);
...
TEST(FTextureSourceTests, ImageRecipeRetainsAlphaPresentOnlyInSuppliedLowerMip)
{
	...
	ASSERT_TRUE(ImageResult8);
	Mips[0] = std::move(*ImageResult8);
	auto ImageResult9 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(4, std::byte{0}));
...
TEST(FTextureSourceTests, SourcePreparationFailurePreservesInstalledState)
{
	...
	auto ImageResult10 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(4, std::byte{1}));
	EXPECT_TRUE(ImageResult10);
	if (ImageResult10) ImportedImage = std::move(*ImageResult10);
	...
	auto ImageResult11 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(3, std::byte{2}));
	EXPECT_FALSE(ImageResult11);
...
TEST(FTextureSourceTests, TextureOwnsSourceAndBuildInputCapturesIdentity)
{
	...
	EXPECT_TRUE(ImageResult12);
	if (ImageResult12) ImportedImage = std::move(*ImageResult12);
	Durin::FTextureSource Imported;
	EXPECT_TRUE(Imported.Init2D(ImportedImage.GetView(), 4));
	Texture->SetSource(Imported);
	EXPECT_EQ(Texture->GetSource().GetOwner(), Texture);
...
TEST(FTextureSourceTests, ImageBuildInputSurvivesSourceReplacementWithoutPixelCopies)
{
	...
	auto ImageResult13 = Durin::Image::FImage::TryCreate({.Width = 2, .Height = 2,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(16, std::byte{17}));
	ASSERT_TRUE(ImageResult13);
	auto PixelsImage = std::move(*ImageResult13);
...
TEST(FTextureSourceTests, RebuildPreservesAuthoredStorageAndRejectsMismatchedReplacement)
{
	...
	Durin::Image::FImage Image;
	auto ImageResult15 = Durin::Image::FImage::TryCreate({.Width = 4, .Height = 4,
		.Format = Durin::Image::ERawImageFormat::RGBA8,
		.GammaSpace = Durin::Image::EImageGammaSpace::SRGB}, Durin::FByteBuffer(64, std::byte{255}));
	ASSERT_TRUE(ImageResult15);
	Image = std::move(*ImageResult15);
	Durin::FTextureSource Source;
	...
	auto ImageResult16 = Durin::Image::FImage::TryCreate({.Width = 4, .Height = 4,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(64, std::byte{41}));
	ASSERT_TRUE(ImageResult16);
	...
	EXPECT_EQ(Rejected.error().ExpectedSourceIdentity, Identity);
	EXPECT_EQ(Rejected.error().ActualSourceIdentity, Different.GetIdentity());
	EXPECT_EQ(Rejected.error().ObjectPath, Texture->GetObjectPath());
	EXPECT_EQ(Texture->GetSource().GetIdentity(), Identity);
...
TEST(FTexture2DTests, CompilationAdmissionRetainsInputCauseWithoutPublishing)
{
	InitializeDObjectSystem();
	ASSERT_TRUE(EnsureTextureCompilingManager());
	auto* Texture = Durin::NewObject<Durin::DTexture2D>(nullptr, "TypedCompilationAdmission");
	ASSERT_NE(Texture, nullptr);
	const auto ObjectPath = Texture->GetObjectPath();
	uint32 Completions = 0;
	const auto Empty = Durin::SubmitTexture2DCompilation(*Texture, {},
		[&](Durin::FTexture2DCompilationResult) { ++Completions; });
	EXPECT_FALSE(Empty);
	EXPECT_EQ(Empty.error().Code, Durin::ETexture2DCompilationError::InvalidSource);
	ASSERT_TRUE(Empty.error().InputCause);
	EXPECT_EQ(Empty.error().InputCause->Code, Durin::ETexture2DInputError::EmptyMips);
	EXPECT_EQ(Empty.error().ObjectPath, ObjectPath);
	EXPECT_FALSE(Durin::HasPendingTexture2DCompilation(*Texture));
	EXPECT_EQ(Completions, 0u);
	EXPECT_FALSE(Texture->HasPlatformData());

	Durin::Image::FImage Image;
	auto ImageResult17 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(4));
	ASSERT_TRUE(ImageResult17);
...
TEST(FTexture2DTests, TerminalRequestsRetireObjectRecordsAndBoundDiagnostics)
{
	...
	for (uint32 Index = 0; Index < PlatformData; ++Index)
	{
		...
		auto ImageResult20 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
			.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(4));
		EXPECT_TRUE(ImageResult20);
		if (ImageResult20) SourceImage = std::move(*ImageResult20);
		Durin::FTextureSource Source;
...
TEST(FTexture2DTests, PlatformCacheIsDeferredIdempotentAndFinishesOnlySelectedTexture)
{
	...
	auto MakeTexture = [](const char* Name, uint32 Width) {
		...
		Durin::Image::FImage Image;
		auto ImageResult21 = Durin::Image::FImage::TryCreate({.Width = Width, .Height = 1,
			.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(Width * 4));
		EXPECT_TRUE(ImageResult21);
		if (ImageResult21) Image = std::move(*ImageResult21);
		Durin::FTextureSource Source;
...
TEST(FTexture2DTests, PendingLimitIncludesFinishedComputesUntilDeliveryReturns)
{
	...
	auto MakeRequest = [] {
		Durin::Image::FImage SourceImage;
		auto ImageResult22 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
			.Format = Durin::Image::ERawImageFormat::RGBA8}, Durin::FByteBuffer(4));
		EXPECT_TRUE(ImageResult22);
		if (ImageResult22) SourceImage = std::move(*ImageResult22);
		Durin::FTextureSource Source;
		EXPECT_TRUE(Source.Init2D(SourceImage.GetView(), 4));
		...
		ASSERT_TRUE(Durin::SubmitTexture2DCompilation(*Texture, MakeRequest(),
			[&](Durin::FTexture2DCompilationResult Result) {
				...
				if (++CompletionCount == 1)
				{
					...
					EXPECT_FALSE(Rejected);
					EXPECT_EQ(Rejected.error().Code, Durin::ETexture2DCompilationError::AdmissionRejected);
...
TEST(FTexture2DTests, SamePathReplacementCannotReceiveDestroyedOwnerCompletion)
{
	...
	auto MakeRequest = [](uint64 Hash) {
		...
		Durin::Image::FImage SourceImage;
		auto ImageResult23 = Durin::Image::FImage::TryCreate({.Width = 1, .Height = 1,
			.Format = Durin::Image::ERawImageFormat::RGBA8}, std::move(SourcePixels));
		EXPECT_TRUE(ImageResult23);
		if (ImageResult23) SourceImage = std::move(*ImageResult23);
		Durin::FTextureSource Source;
		EXPECT_TRUE(Source.Init2D(SourceImage.GetView(), 4));
		return Durin::FTexture2DCompilationRequest{
			.Build = Durin::MakeTexture2DBuildRequest(Source, {.Usage = static_cast<Durin::ETextureUsage>(255)}),
...
TEST(FVolumeTextureTests, PackageReloadCookAndFailedReplacementAreTransactional)
{
	...
	Texture = nullptr;
	const auto Loaded = Durin::LoadObject<Durin::DVolumeTexture>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
	Texture = Loaded.value_or(nullptr);
	...
	ASSERT_TRUE(Durin::ContributeEngineCookAsset(
		*Texture, "/Game/CookedVolume", Cook)) << Error;
	...
	const auto CookedLoad =
		Durin::LoadObject<Durin::DVolumeTexture>(Durin::Testing::MakeTopLevelAssetObjectPathForTests(
			CookedPath, AssetPath.GetPackageName()));
		CookedTexture = CookedLoad.value_or(nullptr);
	ASSERT_TRUE(CookedLoad) << (CookedLoad ? std::string{} : CookedLoad.error().Message);
...
TEST(FVolumeTextureTests, Large128CubedSourcePlansSavesAndReloadsAsAtomicBulkData)
{
	...
	{
		auto ValueResult = Durin::InspectEditorBulkDataCompanionPaths(SavedData->PhysicalPath, Inspection);
		ASSERT_TRUE(ValueResult);
		EditorBulkDataFiles = std::move(*ValueResult);
	}
	...
	const auto MissingLoad =
		Durin::LoadObject<Durin::DVolumeTexture>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
		Texture = MissingLoad.value_or(nullptr);
	...
	const auto CorruptLoad =
		Durin::LoadObject<Durin::DVolumeTexture>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
		Texture = CorruptLoad.value_or(nullptr);
	...
	const auto WarmLoaded =
		Durin::LoadObject<Durin::DVolumeTexture>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
		Texture = WarmLoaded.value_or(nullptr);
	ASSERT_TRUE(WarmLoaded) << (WarmLoaded ? std::string{} : WarmLoaded.error().Message);
	ASSERT_NE(Texture, nullptr);
...
TEST(FTexture2DTests, StandardTranslationFeedsDetachedNormalizedBuildProduct)
{
	...
	{
		auto SourceDataResult = Durin::AssetForge::Builtins::TranslateTexture2DSource(TransparentPngData);
...
TEST(FTexture2DTests, DdcStoreFailureKeepsCompleteProductAndReportsDiagnostic)
{
	...
	InitializeTextureImportMount();
	Durin::FPackagePath Path;
	ASSERT_TRUE(Durin::FPackagePath::TryCreate("/TextureImportTests/CacheWarningCompletion", Path));
	Durin::DTexture2D* Texture = nullptr;
	ASSERT_TRUE(Durin::CreatePackageLeafAssetForTesting(Path, Texture));
	ASSERT_NE(Texture, nullptr);
	std::optional<Durin::FTexture2DCompilationResult> Completion;
	...
	ASSERT_TRUE(Durin::SubmitTexture2DCompilation(*Texture, {
		...
		.Priority = Durin::ETexture2DCompilationPriority::Interactive},
		[&](Durin::FTexture2DCompilationResult Result) { ++CompletionCount; Completion = std::move(Result); }));
...
TEST(FTexture2DTests, CanonicalImportedPixelsRoundTripThroughExternalAuthoredBulk)
{
	...
	{
		auto ValueResult = Durin::InspectEditorBulkDataCompanionPaths(Entry->PhysicalPath, Inspection);
		ASSERT_TRUE(ValueResult);
	...
	const auto Loaded =
		...
		LoadedTexture = Loaded.value_or(nullptr);
	ASSERT_TRUE(Loaded) << (Loaded ? std::string{} : Loaded.error().Message);
	...
	const auto WarmLoaded =
		Durin::LoadObject<Durin::DTexture2D>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
		LoadedTexture = WarmLoaded.value_or(nullptr);
	ASSERT_TRUE(WarmLoaded) << (WarmLoaded ? std::string{} : WarmLoaded.error().Message);
	ASSERT_NE(LoadedTexture, nullptr);
	const Durin::FPackageResourceHandle WarmResource =
		Durin::GetPackageResourceManager().FindPackage(AssetPath.ToString());
	...
	const auto Rejected =
		Durin::LoadObject<Durin::DTexture2D>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
		LoadedTexture = Rejected.value_or(nullptr);
	EXPECT_FALSE(Rejected);
	...
	const auto Missing =
		Durin::LoadObject<Durin::DTexture2D>(Durin::Testing::MakePackageLeafAssetObjectPathForTests(AssetPath));
		LoadedTexture = Missing.value_or(nullptr);
	EXPECT_FALSE(Missing);
	EXPECT_EQ(LoadedTexture, nullptr);
	ASSERT_TRUE(Durin::FFileHelper::SaveArrayToFile(
...
{
	InitializeDObjectSystem();
	auto* Unpackaged = Durin::NewObject<Durin::DTexture2D>(nullptr, "UnpackagedRebuild");
	const auto RejectedRebuild = Durin::AssetForge::Builtins::RebuildTexture2DFromSource(*Unpackaged, {});
	EXPECT_FALSE(RejectedRebuild);
	EXPECT_EQ(RejectedRebuild.error().Code, Durin::ETexture2DCompilationError::MissingPackage);
	EXPECT_EQ(RejectedRebuild.error().ObjectPath, Unpackaged->GetObjectPath());
	FScopedDerivedDataCacheRoot CacheRoot(
	...
	std::string Error;
	{
		auto FailedSourceResult = Durin::AssetForge::Builtins::TranslateTexture2DSource(Encoded);
		ASSERT_TRUE(FailedSourceResult);
		FailedSource = std::move(*FailedSourceResult);
	}
	...
	ASSERT_TRUE(Durin::SubmitTexture2DCompilation(
		*Imported.Asset, std::move(FailedRequest),
		[&](Durin::FTexture2DCompilationResult Result) {
	...
	{
		auto FirstSourceResult = Durin::AssetForge::Builtins::TranslateTexture2DSource(Encoded);
		ASSERT_TRUE(FirstSourceResult);
		FirstSource = std::move(*FirstSourceResult);
	}
	{
		auto SecondSourceResult = Durin::AssetForge::Builtins::TranslateTexture2DSource(Encoded);
		ASSERT_TRUE(SecondSourceResult);
		SecondSource = std::move(*SecondSourceResult);
	}
	std::optional<Durin::FTexture2DCompilationResult> FirstResult;
	std::optional<Durin::FTexture2DCompilationResult> SecondResult;
	int32 FirstCompletionCount = 0;
	ASSERT_TRUE(Durin::SubmitTexture2DCompilation(
		*Imported.Asset, MakeRequest(std::move(FirstSource)),
		[&](Durin::FTexture2DCompilationResult Result) {
			++FirstCompletionCount;
			FirstResult = std::move(Result);
		}));
	ASSERT_TRUE(Durin::SubmitTexture2DCompilation(
		*Imported.Asset, MakeRequest(std::move(SecondSource)),
		[&](Durin::FTexture2DCompilationResult Result) {
	...
	EXPECT_EQ(FirstResult->Error.Code, Durin::ETexture2DCompilationError::Superseded);
	EXPECT_EQ(FirstResult->Error.ObjectPath, Imported.Asset->GetObjectPath());
	ASSERT_TRUE(Durin::WaitForTexture2DCompilation(*Imported.Asset, 10.0));
...
TEST(FTexture2DTests, PreservesMaskedAlphaCoverageWithoutChangingColor)
{
	...
	auto ImageResult24 = Durin::Image::FImage::TryCreate({.Width = 8, .Height = 8,
		.Format = Durin::Image::ERawImageFormat::RGBA8}, std::move(Pixels));
	ASSERT_TRUE(ImageResult24);
...
TEST(FTexture2DTests, SharedSourceSlicesStayImmutableDuringWorkerBuild)
{
	...
	auto ImageResult26 = Image::FImage::TryCreate({.Width = 9, .Height = 7,
		.Format = Image::ERawImageFormat::RGBA8}, Storage.MakeView(11, 9 * 7 * 4));
	ASSERT_TRUE(ImageResult26);
	Images[0] = std::move(*ImageResult26);
	auto ImageResult27 = Image::FImage::TryCreate({.Width = 4, .Height = 3,
		.Format = Image::ERawImageFormat::RGBA8}, Storage.MakeView(11 + 9 * 7 * 4, 4 * 3 * 4));
	ASSERT_TRUE(ImageResult27);
	Images[1] = std::move(*ImageResult27);
...
TEST(FTexture2DTests, ParallelCompressionMatchesSerialBytes)
{
	...
	auto ImageResult28 = Image::FImage::TryCreate({.Width = 257, .Height = 259,
		.Format = Image::ERawImageFormat::RGBA8}, Pixels);
	ASSERT_TRUE(ImageResult28);
	auto Source = std::move(*ImageResult28);
...
TEST(FTexture2DTests, TranslationReturnsOwnedSourceAndRetainsDecodeCause)
{
	Durin::FTextureSource Source;
	using namespace Durin::AssetForge::Builtins;
	{
		auto SourceResult = TranslateTexture2DSource(std::as_bytes(std::span{TransparentPngBytes}));
		ASSERT_TRUE(SourceResult);
		Source = std::move(*SourceResult);
	}
	ASSERT_TRUE(Source.IsValid());
	const auto Empty = TranslateTexture2DSource({});
	ASSERT_FALSE(Empty);
	EXPECT_TRUE(Source.IsValid());
	EXPECT_EQ(Empty.error().Code, ETexture2DTranslationError::Decode);
	ASSERT_TRUE(Empty.error().DecodeCause);
	EXPECT_EQ(Empty.error().DecodeCause->Code, Durin::Image::EImageDecodeError::Empty);
	const std::array<std::byte, 3> Bytes{std::byte{1}, std::byte{2}, std::byte{3}};
	const auto Corrupt = TranslateTexture2DSource(Bytes);
	ASSERT_FALSE(Corrupt);
	EXPECT_EQ(Corrupt.error().Code, ETexture2DTranslationError::Decode);
	ASSERT_TRUE(Corrupt.error().DecodeCause);
	EXPECT_EQ(Corrupt.error().DecodeCause->Code, Durin::Image::EImageDecodeError::InvalidImage);
	EXPECT_EQ(Corrupt.error().DecodeCause->EncodedBytes, Bytes.size());
	EXPECT_EQ(Empty.error().DecodeCause->Code, Durin::Image::EImageDecodeError::Empty);
}

TEST(FTexture2DTests, PropertySettersRetainInvalidSettingsAndCompilationCauses)
{
	InitializeDObjectSystem();
	using namespace Durin;
	using namespace Durin::AssetForge::Builtins;
	auto* Texture = NewObject<DTexture2D>(nullptr, "InvalidTextureSettings", EObjectFlags::Transient);
	const auto OriginalUsage = Texture->GetUsage();
	const auto OriginalQuality = Texture->GetCompressionQuality();
	const auto OriginalMode = Texture->GetAlphaMipMode();
	const auto OriginalThreshold = Texture->GetAlphaCoverageThreshold();
	const auto Usage = SetTexture2DUsage(*Texture, static_cast<ETextureUsage>(255));
	const auto Quality = SetTexture2DCompressionQuality(*Texture, static_cast<ETextureCompressionQuality>(255));
	const auto Mode = SetTexture2DAlphaMipMode(*Texture, static_cast<ETextureAlphaMipMode>(255));
	const auto Threshold = SetTexture2DAlphaCoverageThreshold(*Texture, -1.0f);
	for (const auto* Result : {&Usage, &Quality, &Mode, &Threshold})
	{
		EXPECT_FALSE(*Result);
		EXPECT_EQ(Result->error().Code, ETexture2DCompilationError::InvalidSettings);
		EXPECT_EQ(Result->error().ObjectPath, Texture->GetObjectPath());
		ASSERT_TRUE(Result->error().InputCause);
	}
	EXPECT_EQ(Usage.error().InputCause->Code, ETexture2DInputError::InvalidUsage);
	EXPECT_EQ(static_cast<uint8>(Usage.error().InputCause->Settings.Usage), 255u);
	EXPECT_EQ(Quality.error().InputCause->Code, ETexture2DInputError::InvalidCompressionQuality);
	EXPECT_EQ(static_cast<uint8>(Quality.error().InputCause->Settings.CompressionQuality), 255u);
	EXPECT_EQ(Mode.error().InputCause->Code, ETexture2DInputError::InvalidAlphaMipMode);
	EXPECT_EQ(static_cast<uint8>(Mode.error().InputCause->Settings.AlphaMipMode), 255u);
	EXPECT_EQ(Threshold.error().InputCause->Code, ETexture2DInputError::InvalidAlphaCoverageThreshold);
	EXPECT_EQ(Threshold.error().InputCause->Settings.AlphaCoverageThreshold, -1.0f);
	EXPECT_EQ(Texture->GetUsage(), OriginalUsage);
	EXPECT_EQ(Texture->GetCompressionQuality(), OriginalQuality);
	EXPECT_EQ(Texture->GetAlphaMipMode(), OriginalMode);
	EXPECT_EQ(Texture->GetAlphaCoverageThreshold(), OriginalThreshold);
	EXPECT_TRUE(SetTexture2DSRGB(*Texture, Texture->IsSRGB()));
	const auto MissingPackage = SetTexture2DSRGB(*Texture, !Texture->IsSRGB());
	EXPECT_FALSE(MissingPackage);
	EXPECT_EQ(MissingPackage.error().Code, ETexture2DCompilationError::MissingPackage);
}
