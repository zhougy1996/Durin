...
TEST(FVolumeTextureSourceImportTests, SourceCaptureRetainsTypedFailuresAndClearsOutput)
{
	...
	const auto Snapshot = CaptureEncodedSource("source.bin", Path);
	...
	const auto TooLarge = CaptureEncodedSource(Filename, Path, 2);
	Filename.clear();
	...
	const auto Missing = CaptureEncodedSource("missing.bin", Path.string() + ".missing");
