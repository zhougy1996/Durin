...
#if DURIN_WITH_EDITOR
TEST(FWorldTests, PrimitiveSceneChangesTrackComponentsWithoutPickingGeometry)
{
	Durin::DWorld* World = CreateWorld();
	auto* Actor = World->SpawnActor<Durin::AStaticMeshActor>("Observed");
	auto* Component = Actor->GetStaticMeshComponent();
	auto* Level = World->GetCurrentLevel();
	auto& Changes = Level->GetPrimitiveSceneChanges();
	std::vector<Durin::FPrimitiveSceneMutationBatch> Batches;
	const auto Subscription = Changes.Subscribe([&](const auto& Batch) { Batches.push_back(Batch); });
	ASSERT_NE(Subscription, 0u);
	ASSERT_EQ(Batches.size(), 1u);
	EXPECT_TRUE(Batches.front().bCompleteSnapshot);
	ASSERT_EQ(Batches.front().Mutations.size(), 1u);
	EXPECT_EQ(Batches.front().Mutations.front().Component.Get(), Component);
	const auto Identity = Batches.front().Mutations.front().PrimitiveId;
	const auto Generation = Batches.front().Mutations.front().RegistrationGeneration;
	Component->SetWorldLocation({10.0, 0.0, 0.0});
	Component->UnregisterComponent();
	Component->RegisterComponent();
	ASSERT_GE(Batches.size(), 4u);
	for (size_t Index = 1; Index < Batches.size(); ++Index)
	{
		EXPECT_EQ(Batches[Index].Revision, Batches[Index - 1].Revision + 1);
		EXPECT_FALSE(Batches[Index].bCompleteSnapshot);
		ASSERT_EQ(Batches[Index].Mutations.size(), 1u);
		EXPECT_EQ(Batches[Index].Mutations.front().PrimitiveId, Identity);
	}
	EXPECT_TRUE(Batches[Batches.size() - 2].Mutations.front().bRetired);
	EXPECT_FALSE(Batches.back().Mutations.front().bRetired);
	EXPECT_NE(Batches.back().Mutations.front().RegistrationGeneration, Generation);
	const auto Snapshot = Changes.CaptureSnapshot();
	EXPECT_EQ(Snapshot.Revision, Batches.back().Revision);
	EXPECT_TRUE(Snapshot.bCompleteSnapshot);
	Changes.Unsubscribe(Subscription);
	const auto Count = Batches.size();
	Component->SetWorldLocation({20.0, 0.0, 0.0});
	EXPECT_EQ(Batches.size(), Count);
	Durin::MarkObjectHierarchyAsGarbage(World);
	Durin::CollectGarbage();
}

TEST(FWorldTests, PrimitiveSceneMutationPublishesNestedTransientWeakReflection)
{
	auto* Mutation = Durin::FPrimitiveSceneMutation::StaticStruct();
	auto* Batch = Durin::FPrimitiveSceneMutationBatch::StaticStruct();
