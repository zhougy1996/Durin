...
class TestCore:
    ...
    def test_recover_builds_incrementally_without_cleaning(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        run.assert_called_once_with(['cmake', '--build', str(build_directory), '--target', 'Core', '-j', '4'], environment={'PATH': 'cached'}, output=output, cwd=settings.default_build_paths().root, state_directory=settings.default_build_paths().state_directory)
