...
class TestCore:
    ...
    def test_run_application_waits_for_relaunched_descendants(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        project.touch()
        context = build_config.BuildContext(request_fixtures.command_request(build_config.Action.RUN, options=request_fixtures.RunActionOptions(project_path=project.resolve(), arguments=('--hidden-window', 'argument with spaces'))), build_config.LocalConfig(), self.make_profile(), {'debug': preset}, preset, 'windows')
