...
class TestCore:
    ...
    def test_buildtool_rejects_missing_windows_long_paths_policy(self) -> None:
        ...
        with mock.patch.object(build_toolchain_context.os, 'name', 'nt'), mock.patch.dict(os.sys.modules, {'winreg': fake_winreg}), pytest.raises(errors.BuildToolError, match='LongPathsEnabled'):
    ...
    def test_run_project_is_normalized_and_validated_without_toolchain_state(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        request = request_fixtures.command_request(models.Action.RUN, options=request_fixtures.RunActionOptions(project_path=Path('Games') / '示例 Project' / 'Example.dproject', arguments=('--hidden-window', 'argument with spaces')))
        normalized = request_validation.normalize_run_request(request, root=root)
    ...
    def test_run_defaults_durin_game_to_sandbox_project(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        normalized = request_validation.normalize_run_request(
            request,
            preset=self.make_preset(runtime_variant='DurinGame'),
            root=root,
            default_project=Path('Sandbox/Sandbox.dproject'),
        )
        assert normalized.project_path == descriptor.resolve()
    def test_run_does_not_default_editor_or_override_raw_project_selector(self) -> None:
        request = request_fixtures.command_request(models.Action.RUN, options=request_fixtures.RunActionOptions())
        editor_request = request_validation.normalize_run_request(request, preset=self.make_preset(runtime_variant='DurinEditor'))
        raw_game_request = request_fixtures.command_request(models.Action.RUN, options=request_fixtures.RunActionOptions(arguments=('--project=Other.dproject',)))
        normalized_raw_game_request = request_validation.normalize_run_request(raw_game_request, preset=self.make_preset(runtime_variant='DurinGame'))
    ...
    def test_run_project_rejects_missing_and_wrong_extension_descriptors(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        directory = tmp_path_factory.mktemp('case')
        root = Path(directory)
        wrong_extension = request_fixtures.command_request(models.Action.RUN, options=request_fixtures.RunActionOptions(project_path=Path('Example.json')))
        with pytest.raises(errors.BuildToolError, match='\\.dproject extension'):
            request_validation.normalize_run_request(wrong_extension, root=root)
        missing = request_fixtures.command_request(models.Action.RUN, options=request_fixtures.RunActionOptions(project_path=Path('Missing.dproject')))
        with pytest.raises(errors.BuildToolError, match='was not found'):
            request_validation.normalize_run_request(missing, root=root)
    def test_run_project_rejects_conflicting_runtime_project_selectors(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        for arguments in (('--project', 'Other.dproject'), ('--project=Other.dproject',)):
            with pytest.raises(errors.BuildToolError, match='either through --project or through --args'):
                request_validation.normalize_run_request(request_fixtures.command_request(models.Action.RUN, options=request_fixtures.RunActionOptions(project_path=descriptor, arguments=arguments)))
    ...
    def test_inherit_provider_preserves_environment(self) -> None:
        with mock.patch.dict(os.environ, {'DURIN_TEST_ENV': 'present'}, clear=True):
            environment = build_toolchain_context.build_environment(self.make_profile(), models.EnvironmentSetup(), current_host='windows')
        assert environment['DURIN_TEST_ENV'] == 'present'
    def test_visual_studio_environment_is_captured_once(self) -> None:
        profile = replace(self.make_profile(), environment_provider=models.EnvironmentProvider.VISUAL_STUDIO)
        with mock.patch.object(build_toolchain_context, 'load_visual_studio_environment_cache', return_value=None), mock.patch.object(build_toolchain_context, 'write_visual_studio_environment_cache'), mock.patch.object(build_toolchain_context, 'find_vsdevcmd', return_value=Path('VsDevCmd.bat')), mock.patch.object(build_toolchain_context, 'capture_setup_environment', return_value={'PATH': 'ready', 'VSLANG': '2052'}) as capture, mock.patch.object(build_toolchain_context, 'detect_msvc_showincludes_prefix', return_value='Note: including file:  ') as detect_prefix:
            environment = build_toolchain_context.build_environment(profile, models.EnvironmentSetup(), current_host='windows')
    ...
    def test_visual_studio_environment_cache_reuses_delta_and_invalidates_for_compiler_change(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        with mock.patch.object(build_toolchain_context, 'find_vsdevcmd', return_value=script), mock.patch.object(build_toolchain_context, 'visual_studio_environment_cache_path', return_value=cache), mock.patch.object(build_toolchain_context, 'capture_setup_environment', return_value=captured) as capture, mock.patch.object(build_toolchain_context, 'detect_msvc_showincludes_prefix', return_value='Note: including file:  ') as detect_prefix, mock.patch.object(build_toolchain_context, 'find_command', return_value=str(compiler)), mock.patch.dict(os.environ, {'DURIN_LIVE_VALUE': 'first', 'PATH': 'original-path'}, clear=True):
            first = build_toolchain_context.build_environment(profile, models.EnvironmentSetup(), current_host='windows')
            os.environ['DURIN_LIVE_VALUE'] = 'second'
            os.environ['PATH'] = 'new-path'
            second = build_toolchain_context.build_environment(profile, models.EnvironmentSetup(), current_host='windows')
            compiler.write_text('updated', encoding='utf-8')
            build_toolchain_context.build_environment(profile, models.EnvironmentSetup(), current_host='windows')
    ...
    def test_visual_studio_environment_rejects_localized_compiler_output(self) -> None:
        profile = replace(self.make_profile(), environment_provider=models.EnvironmentProvider.VISUAL_STUDIO)
        with mock.patch.object(build_toolchain_context, 'load_visual_studio_environment_cache', return_value=None), mock.patch.object(build_toolchain_context, 'find_vsdevcmd', return_value=Path('VsDevCmd.bat')), mock.patch.object(build_toolchain_context, 'capture_setup_environment', return_value={'PATH': 'ready'}), mock.patch.object(build_toolchain_context, 'detect_msvc_showincludes_prefix', return_value='注意: 包含文件:  '), pytest.raises(errors.BuildToolError, match='English language pack'):
            build_toolchain_context.build_environment(profile, models.EnvironmentSetup(), current_host='windows')
    ...
    def test_visual_studio_profile_adds_bundled_ninja(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        ninja.touch()
        profile = replace(self.make_profile(), environment_provider=models.EnvironmentProvider.VISUAL_STUDIO, required_commands=('ninja',))
    ...
    def test_derive_context_reuses_toolchain_environment(self) -> None:
        ...
        request = request_fixtures.command_request(models.Action.SHELL, context=requests.RequestContext(preset='debug'))
        environment = {'PATH': 'cached'}
        context = build_context.BuildContext(request, models.LocalConfig(), profile, presets, presets['debug'], 'windows', cmake='cmake', jobs=8, environment=environment, repository=RepositoryContext.load())
        child = build_context.derive_build_context(context, request_fixtures.command_request(models.Action.BUILD, context=requests.RequestContext(preset='release'), options=request_fixtures.BuildActionOptions(target='all')))
